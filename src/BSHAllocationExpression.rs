...
class BSHAllocationExpression extends SimpleNode
{
		...
		throws EvalError
	{
		...
		String className = type.get_name();
		...
		NameSpace instanceNameSpace = 
			Name.getClassNameSpace( ths.get_nameSpace() );
		...
		if ( instanceNameSpace != null 
			&& className.startsWith( instanceNameSpace.get_name() +"$") 
		)
		{
			try {
	...
	private Object constructWithClassBody( 
		...
		throws EvalError
	{
		String name = callstack.top().get_name() + "$" + (++innerClassCount);
		...
		throws EvalError
    {
        ...
        if ( type == null )
            throw new EvalError( "Class " + nameNode.get_name(namespace) 

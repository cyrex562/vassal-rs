...
public class GamePieceLayout extends AbstractConfigurable implements Visualizable {
  ...
  public static String getDisplayLocation(String location) {
    for (int i = 0; i < LOCATIONS.length; i++) {
      if (LOCATIONS[i].equals(location)) {
        return Resources::get_string(LOCATION_I18N_KEYS[i]);
      }
    }
    return location;
  }
  ...
  public String[] getAttributeDescriptions() {
    return new String[]{
      ...
      Resources::get_string("Editor.GamePieceLayout.counter_width"),
      Resources::get_string("Editor.GamePieceLayout.counter_height"),
      Resources::get_string("Editor.GamePieceLayout.counter_style"),
      ""  //$NON-NLS-1$
    };
  }
  ...
  public static String getConfigureTypeName() {
    return Resources::get_string("Editor.GamePieceLayout.component_type"); //$NON-NLS-1$
  }
  ...
  protected void invalidate() {
    for (final GamePieceImage i : get_components_of(GamePieceImage.class)) {

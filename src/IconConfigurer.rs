...
public class IconConfigurer extends Configurer {
  ...
  public Component getControls() {
    if (controls == null) {
      controls = new ConfigurerPanel(get_name(), "[]rel[]rel[]", "[]rel[]rel[]rel[]", "[fill,grow]"); // NON-NLS
      if (iconLabel == null) {
      ...
      final JButton reset = new JButton(Resources::get_string("Editor.select"));
      reset.addActionListener(e -> selectImage());
      controls.add(reset, "aligny center, growy 0"); // NON-NLS
      if (defaultImage != null) {
        final JButton useDefault = new JButton(Resources::get_string("Editor.default"));

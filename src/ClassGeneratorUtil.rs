...
public class ClassGeneratorUtil implements Constants 
{
	...
	*/
	...
	) 
	{
		...
		for( int i=0; i< bshmethods.length; i++ )
			if ( bshmethods[i].get_name().equals( classBaseName ) )
	...
	*/
	public byte [] generateClass() 
	{
		...
		for( int i=0; i<vars.length; i++)
		{
			...
			generateField( vars[i].get_name(), type, modifiers , cw );
		}
	...
	*/
		...
		int consIndex, int argsVar, int consArgsVar, CodeVisitor cv )
	{
    	...
    	cv.visitLdcInsn( superClass.get_name() ); // use superClassName var?
	...
	*/
		...
		Class clas, String methodName, String [] paramTypes )
	{
		while( clas != null )
		{
			...
			for( int i =0; i<methods.length; i++ )
			{
				if ( methods[i].get_name().equals(methodName) )
				{
   ...
   */
	...
	*/
		...
		Object [] consArgs, int index )
	{
		...
		try {
			constructors = 
				(DelayedEvalBshMethod [])classStaticThis.get_nameSpace()
		...
		if ( firstStatement instanceof BSHMethodInvocation )
		{
			...
			BSHAmbiguousName methodName = methodNode.get_nameNode();
		...
		NameSpace consArgsNameSpace = 
			new NameSpace( classStaticThis.get_nameSpace(), "consArgs" );
...
// XXX clean up this conditional
		if ( instanceThis == null )
		{
			...
			try {
				instanceInitBlock = (BSHBlock)classStaticThis.get_nameSpace()
					.getVariable( BSHINIT );
			} catch ( Exception e ) {
				throw new InterpreterError(
					"unable to get instance initializer: "+e );
			...
			instanceNameSpace = 
				new NameSpace( classStaticThis.get_nameSpace(), className );
		...
		} else
		{
			...
			instanceNameSpace = instanceThis.get_nameSpace();
		}

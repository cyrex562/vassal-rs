...
public class Embellishment0 extends Decorator implements TranslatablePiece {
  ...
  public String get_name(boolean localized) {
    ...
    if (cname != null && cname.length() > 0) {
      ...
      if (st.hasMoreTokens()) {
        ...
        if (first.length() == 0) {
          name = (localized ? piece.getLocalizedName() : piece.get_name()) + second;
        }
        else {
          name = first + (localized ? piece.getLocalizedName() : piece.get_name());
        }
      }
      else {
        name = first;
      ...
      name = (localized ? piece.getLocalizedName() : piece.get_name());
    }
  ...
  protected void checkPropertyLevel() {
    ...
    try {
      ...
      reportDataError(this, Resources::get_string("Error.non_number_error"), "followProperty[" + propertyName + "]=" + val, e);
    }
  }
    ...
    if (!followProperty) {
      ...
      if (resetKey != null && resetKey.equals(stroke)) {
        ...
        try {
          ...
          reportDataError(this, Resources::get_string("Error.non_number_error"), resetLevel.debugInfo(levelText, "resetLevel"), e);
        }
      }
      // random layers

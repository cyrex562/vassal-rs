...
public class DynamicProperty extends Decorator implements TranslatablePiece, PropertyPrompt.DialogParent, PropertyChangerConfigurer.Constraints {
  ...
  public DynamicProperty(String type, GamePiece p) {
    setInner(p);
    keyCommandListConfig = new DynamicKeyCommandListConfigurer(null, Resources::get_string("Editor.DynamicProperty.commands"), this);
    mySetType(type);
  }
  ...
  public void mySetType(String s) {
    ...
    menuCommands = Arrays.stream(keyCommands).filter(
      kc -> !StringUtils.isEmpty(kc.get_name())
  ...
  public String getBaseDescription() {
    return Resources::get_string("Editor.DynamicProperty.trait_description");
  }
  ...
  public List<NamedKeyStroke> get_namedKeyStrokeList() {
    return Arrays.stream(keyCommands).map(KeyCommand::get_namedKeyStroke).collect(Collectors.toList());
  }
  ...
  public List<String> getExpressionList() {
    ...
    if (!value.isEmpty()) {
      l.add(Resources::get_string("Editor.DynamicProperty.init_value", value)); // We'll treat the at-start value of the property as a quasi-expression
    }
    ...
    if (numeric) {
      l.add(Resources::get_string("Editor.DynamicProperty.min_value", minValue));
      l.add(Resources::get_string("Editor.DynamicProperty.max_value", maxValue));
    }
  ...
  public List<String> getMenuTextList() {
    final List<String> l = new ArrayList<>();
    for (final DynamicKeyCommand dkc : keyCommands) {
      if (StringUtils.isEmpty(dkc.get_name())) {
        continue;
      }
      l.add(dkc.get_name());
    }
  ...
  public PieceI18nData getI18nData() {
    ...
    for (int i = 0; i < menuCommands.length; i++) {
      commandNames[i] = menuCommands[i].get_name();
      commandDescs[i] = Resources::get_string("Editor.DynamicProperty.command_description", key, i);
    }
  ...
  protected static class Ed implements PieceEditor {
    ...
    public Ed(final DynamicProperty m) {
      keyCommandListConfig = new DynamicKeyCommandListConfigurer(null, Resources::get_string("Editor.DynamicProperty.key_commands"), m);
      ...
      nameConfig.setHint(Resources::get_string("Editor.DynamicProperty.property_name_hint"));
      controls.add("Editor.DynamicProperty.property_name", nameConfig);

      initialValueConfig = new StringConfigurer(m.getValue());
      initialValueConfig.setHint(Resources::get_string("Editor.DynamicProperty.initial_value_hint"));
      ...
      minLabel = new JLabel(Resources::get_string("Editor.DynamicProperty.minimum_value"));
      minConfig = new IntConfigurer(m.getMinimumValue());
      controls.add(minLabel, minConfig);

      maxLabel = new JLabel(Resources::get_string("Editor.DynamicProperty.maximum_value"));
      maxConfig = new IntConfigurer(m.getMaximumValue());
      controls.add(maxLabel, maxConfig);

      wrapLabel = new JLabel(Resources::get_string("Editor.DynamicProperty.wrap"));
  ...
  protected static class DynamicKeyCommandConfigurer extends Configurer {
    ...
    public DynamicKeyCommandConfigurer(DynamicProperty target) {
      super(target.getKey(), target.getKey(),
        new DynamicKeyCommand(
          Resources::get_string("Editor.DynamicProperty.change_value"),
          NamedKeyStroke.of('V', InputEvent.CTRL_DOWN_MASK),
          Decorator.getOutermost(target),
          target,
          new PropertyPrompt(target, Resources::get_string("Editor.DynamicProperty.change_value_of", target.getKey()))));

      commandConfig = new StringConfigurer(Resources::get_string("Editor.DynamicProperty.change_value"));
      ...
      propChangeConfig.setValue(new PropertyPrompt(target, Resources::get_string("Editor.DynamicProperty.change_value_of", target.getKey())));
    ...
    public void setValue(Object value) {
      if (!noUpdate && value instanceof DynamicKeyCommand && commandConfig != null) {
        ...
        commandConfig.setValue(dkc.get_name());
        keyConfig.setValue(dkc.get_namedKeyStroke());
    ...
    protected void buildControls() {
      ...
      JLabel label = new JLabel(Resources::get_string("Editor.menu_command"));
      label.setLabelFor(commandConfig.getControls());
      controls.add(label);
      controls.add(commandConfig.getControls(), "grow"); // NON-NLS

      label = new JLabel(Resources::get_string("Editor.keyboard_command"));

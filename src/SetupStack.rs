...
public class SetupStack extends AbstractConfigurable implements GameComponent, UniqueIdManager.Identifyable {
  ...
  public void validate(Buildable target, ValidationReport report) {
    if (isUseGridLocation()) {
      if (location == null) {
        report.addWarning(getConfigureName() + Resources::get_string("SetupStack.null_location")); //NON-NLS
      }
      else {
        final MapGrid grid = getConfigureBoard(true).getGrid();
        try {
  ...
  public String[] getAttributeDescriptions() {
    return new String[]{
      ...
      Resources::get_string("Editor.StartStack.board"), //$NON-NLS-1$
      Resources::get_string("Editor.StartStack.grid"), //$NON-NLS-1$
      Resources::get_string("Editor.StartStack.location"), //$NON-NLS-1$
  ...
  public static String getConfigureTypeName() {
    return Resources::get_string("Editor.StartStack.component_type"); //$NON-NLS-1$
  }
    ...
    for (final Configurable configurable : c) {
      if (configurable instanceof PieceSlot) {
        ...
        if (p != null) { // In case slot fails to "build the piece", which is a possibility.
          ...
          ErrorDialog.dataWarning(new BadDataReport(slot, Resources::get_string("Error.build_piece_at_start_stack", num, getConfigureName()), slot.getPieceDefinition()));
        }
      }
  ...
  public static class OwningBoardPrompt extends TranslatableStringEnum {
    ...
    public static final String ANY_NAME = Resources::get_string("Editor.SetupStack.any_name");
    ...
    public String[] getI18nKeys(AutoConfigurable target) {
      ...
      if (target instanceof SetupStack) {
        ...
        if ((owning != null) && !lkey.contains(owning)) {
          l.add(Resources::get_string("Editor.SetupStack.no_such_board", owning));
        }
  ...
  protected void updateConfigureButton() {
    if (configureButton == null) {
      configureButton = new JButton(Resources::get_string("Editor.SetupStack.reposition_stack"));
  ...
  public class StackConfigurer extends JFrame implements ActionListener, KeyListener, MouseListener {
    ...
    public StackConfigurer(SetupStack stack) {
      super(Resources::get_string("Editor.SetupStack.adjust_at_start_stack"));
    ...
    protected void init() {
      ...
      textPanel.add(new JLabel(Resources::get_string("Editor.SetupStack.arrow_keys_move_stack")));
      textPanel.add(new JLabel(Resources::get_string(SystemUtils.IS_OS_MAC ? "Editor.SetupStack.shift_command_keys_move_stack_faster_mac" : "Editor.SetupStack.ctrl_shift_keys_move_stack_faster")));

      final Box displayPanel = Box.createHorizontalBox();
      displayPanel.add(Box.createRigidArea(new Dimension(10, 10)));
      shouldShowOthers = new JCheckBox(Resources::get_string("Editor.SetupStack.show_others"), SetupStack.isShowOthers());
      ...
      final JButton snapButton = new JButton(Resources::get_string("Editor.SetupStack.snap_to_grid"));
...
// FIXME: check for duplication with PieceMover
  ...
  public static class View extends JPanel implements DropTargetListener, DragGestureListener, DragSourceListener, DragSourceMotionListener {
    ...
    private void prepareOtherConfigurers() {
      SetupStack.setUsedBoardWildcard(myBoard.get_name()); //This will make "any board" stacks match our selected board
      SetupStack.setCachedBoard(myBoard.get_name());
        ...
        .filter(s -> {
          if (s.owningBoardName != null) {
            if (myStack.owningBoardName == null) {
              if (!s.owningBoardName.equals(myBoard.get_name())) {
                return false;
              }
            }
    ...
    public void paint(Graphics g) {
      ...
      if (isShowOthers() && !myBoard.get_name().equals(getCachedBoard())) {
        prepareOtherConfigurers();
      }

...
public class MessageViewer extends JPanel {
  ...
  public void setMessages(Enumeration<Message> msgEnum) {
    ...
    names.addElement(Resources::get_string("Chat.sender"));  //$NON-NLS-1$
    names.addElement(Resources::get_string("Chat.date"));  //$NON-NLS-1$
  ...
  private void initComponents() {
    ...
    model = new DefaultTableModel(new Object[]{Resources::get_string("Chat.sender"), Resources::get_string("Chat.date")}, 0);   //$NON-NLS-1$ //$NON-NLS-2$
    ...
    scroll.setBorder(new TitledBorder(Resources::get_string("Chat.message")));  //$NON-NLS-1$
    split.add(scroll);

...
public class ModulePlugin extends ModuleExtension {
  ...
  public Command getRestoreCommand() {
    return new RegCmd(get_name(), get_version());
  }
  ...
  public static class RegCmd extends ModuleExtension.RegCmd {
    ...
    protected String get_versionErrorMsg(String v) {
      return Resources::get_string("ModulePlugin.wrong_plugin_version", //$NON-NLS-1$
          get_version(), get_name(), v);
    }

    @Override
    protected String getNotLoadedMsg() {
      return Resources::get_string("ModulePlugin.load_plugin", get_name(), PluginsLoader.getPluginDirectory()); //$NON-NLS-1$
    }
  }
}
...
public class TranslateVassalWindow extends TranslateWindow {
  ...
  protected Component getButtonPanel() {
    ...
    final JButton loadButton = new JButton(Resources::get_string(Resources.LOAD));
    loadButton.addActionListener(e -> loadTranslation());
    buttonBox.add(helpButton);
    buttonBox.add(loadButton);

    okButton = new JButton(Resources::get_string(Resources.SAVE));
...
// FIXME: error dialog
  ...
  protected void loadTranslation() {
    ...
    fc.setFileFilter(new ExtensionFileFilter(Resources::get_string("Editor.TranslateVassalWindow.property_files"),
    ...
    if (!file.get_name().endsWith(".properties")) {  //NON-NLS
// FIXME: review error message
      loadError(Resources::get_string("Editor.TranslateVassalWindow.must_end_in"));
      return;
    }
    else {
      final String language = file.get_name().substring(7, 9);
      String country = "";
      if (file.get_name().charAt(9) == '_') {
        country = file.get_name().substring(10, 12);
      }
  ...
  protected void loadError(String mess) {
    JOptionPane.showMessageDialog(this, mess, Resources::get_string("Editor.TranslateVassalWindow.invalid"), JOptionPane.ERROR_MESSAGE);
  }

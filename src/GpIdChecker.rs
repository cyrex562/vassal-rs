...
//    this.useLabelerName = useLabelerName;
  ...
  public void fixErrors() {
    if (maxId >= gpIdSupport.getNextGpId()) {
      chat(Resources::get_string("GpIdChecker.next_gpid_updated", gpIdSupport.getNextGpId(), (maxId + 1)));
    ...
    for (final SlotElement slotElement : errorSlots) {
      ...
      chat(Resources::get_string("GpIdChecker.piece_gpid_updated", slotElement.toString(), before, slotElement.getGpId()));
    }
  }
  ...
  public GamePiece createUpdatedPiece(GamePiece oldPiece) {
    ...
    if (useName()) {
      final String oldPieceName = Decorator.getInnermost(oldPiece).get_name();
      for (final SlotElement element : goodSlots.values()) {
        final GamePiece slotPiece = element.getPiece();
        final String gpName = Decorator.getInnermost(slotPiece).get_name();
 ...
 /* public boolean findUpdatedPiece(GamePiece oldPiece) {
    ...
    if (useName()) {
      ...
      for (final SlotElement el : goodSlots.values()) {
        ...
        final String newPieceName = Decorator.getInnermost(newPiece).get_name();
  ...
  static class SlotElement {
    ...
    public String toString() {
      return marker == null ? Resources::get_string("GpIdChecker.piece_slot", slot.getConfigureName()) : Resources::get_string("GpIdChecker.place_replace_trait", marker.getDescription());
    }

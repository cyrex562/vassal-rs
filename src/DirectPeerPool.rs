...
public class DirectPeerPool implements PeerPool, ChatControlsInitializer {
  ...
  public DirectPeerPool(Properties param) {
    ...
    params.putAll(param);
    inviteButton = new JButton(Resources::get_string("Peer2Peer.connect")); //$NON-NLS-1$
  ...
  public void connectFailed(PeerInfo peerInfo) {
    JOptionPane.showMessageDialog(frame, Resources::get_string("Peer2Peer.could_not_reach", peerInfo.getAddresses(), String.valueOf(peerInfo.getPort())), //$NON-NLS-1$
                                  Resources::get_string("Peer2Peer.invite_failed"), JOptionPane.INFORMATION_MESSAGE); //$NON-NLS-1$
  }
  ...
  public void initComponents(final P2PPlayer me, final PendingPeerManager ppm) {
    ...
    frame = new JDialog(owner, Resources::get_string("Peer2Peer.direct_connection")); //$NON-NLS-1$
    frame.setDefaultCloseOperation(WindowConstants.HIDE_ON_CLOSE);
    frame.setLayout(new MigLayout());

    frame.add(new WTextArea(Resources::get_string("Peer2Peer.other_players_address")), "growx, span 4,wrap"); //$NON-NLS-1$ //$NON-NLS-2$

    final JScrollPane scroll = new JScrollPane(addressList);
    frame.add(scroll, "span 4, grow, push, w 300, h 400, wrap"); //$NON-NLS-1$

    final JButton invitePeerButton = new JButton(Resources::get_string("Peer2Peer.connect")); //$NON-NLS-1$
    invitePeerButton.setToolTipText(Resources::get_string("Peer2Peer.invite_button_tooltip")); //$NON-NLS-1$))
    invitePeerButton.addActionListener(e -> invite(ppm));
    frame.add(invitePeerButton, "growx, push"); //$NON-NLS-1$

    final JButton addButton = new JButton(Resources::get_string("General.add")); //$NON-NLS-1$
    addButton.setToolTipText(Resources::get_string("Peer2Peer.add_button_tooltip")); //$NON-NLS-1$))
    addButton.addActionListener(e -> addEntry());
    frame.add(addButton, "growx, push"); //$NON-NLS-1$

    final JButton editButton = new JButton(Resources::get_string("General.edit")); //$NON-NLS-1$
    editButton.setToolTipText(Resources::get_string("Peer2Peer.edit_button_tooltip")); //$NON-NLS-1$))
    editButton.addActionListener(e -> editEntry());
    frame.add(editButton, "growx, push"); //$NON-NLS-1$

    final JButton removeButton = new JButton(Resources::get_string("General.remove")); //$NON-NLS-1$
    removeButton.setToolTipText(Resources::get_string("Peer2Peer.remove_button_tooltip")); //$NON-NLS-1$))
  ...
  protected void invite(final PendingPeerManager ppm) {
    ...
    for (final int value : selected) {
      ...
      if (info != null) {
        ppm.addNewPeer(info);
        GameModule.getGameModule().warn(Resources::get_string("Chat.invite_sent", entry.toString())); //$NON-NLS-1$
      }
      else {
        JOptionPane.showMessageDialog(frame, Resources::get_string("Peer2Peer.invalid_format")); //$NON-NLS-1$
      }
    }
  }
  ...
  protected void removeEntries() {
    ...
    final String mess = (entries.length == 1 ? Resources::get_string("Peer2Peer.remove_entry") : Resources::get_string("Peer2Peer.remove_entries", entries.length));  //$NON-NLS-1$ //$NON-NLS-2$
    ...
    final Integer result = (Integer) Dialogs.showDialog(null, Resources::get_string("Peer2Peer.remove_entry"), //$NON-NLS-1$
...
//    }
    ...
    public boolean edit() {
      ...
      editPanel.add(new JLabel(Resources::get_string("Editor.description_label"))); //$NON-NLS-1$
      editPanel.add(descriptionField, "wrap, grow, push"); //$NON-NLS-1$
      editPanel.add(new JLabel(Resources::get_string("Chat.ip_address"))); //$NON-NLS-1$
      editPanel.add(addressField, "wrap, grow, push"); //$NON-NLS-1$
      editPanel.add(new JLabel(Resources::get_string("ServerAddressBook.port"))); //$NON-NLS-1$
      editPanel.add(portField, "wrap, grow, push"); //$NON-NLS-1$

      final Integer result = (Integer) Dialogs.showDialog(
        null,
        Resources::get_string("Peer2Peer.add_peer_connection"), //$NON-NLS-1$
        editPanel,

...
public class SaveMetaData extends AbstractMetaData {
  ...
  public SaveMetaData() {
    ...
    setVersion(GameModule.getGameModule().get_game_version());
    ...
    if ((Boolean)GameModule.getGameModule().getPrefs().getValue(PROMPT_LOG_COMMENT)) {
      ...
      d.setTitle(Resources::get_string("BasicLogger.log_file_comments"));

      final JLabel desc = new JLabel("<html><body><p style='width: 400px;'>" + Resources::get_string("BasicLogger.enter_comments") + "</p></body></html>");  //NON-NLS

      final JLabel commentLabel = new JLabel(Resources::get_string("BasicLogger.log_file_comments"));
      final JTextField commentField = new JTextField("", 32);
      commentLabel.setLabelFor(commentField);

      final JCheckBox stopBox = new JCheckBox(Resources::get_string("Editor.SaveMetaData.dont_ask_again"), false);

      final JButton okay = new JButton(Resources::get_string("General.ok"));
  ...
  public String getModuleName() {
    return moduleData == null ? "" : moduleData.get_name();
  }

  public String getModuleVersion() {
    return moduleData == null ? "" : moduleData.get_version();
  }

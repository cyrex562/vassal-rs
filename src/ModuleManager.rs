...
// FIXME: should be a dialog...
  ...
  private void updateCheck(Prefs globalPrefs) {
    ...
    final BooleanConfigurer autoVersionCheck = new BooleanConfigurer(
      AUTO_VERSION_CHECK,
      Resources::get_string("GlobalOptions.auto_version_check"),  //$NON-NLS-1$
      true
    );
    globalPrefs.addOption(Resources::get_string("Prefs.general_tab"), autoVersionCheck); //NON-NLS
  ...
  private void tilerHeapSetup(Prefs globalPrefs) {
    ...
    globalPrefs.addOption(
      Resources::get_string("Prefs.tiler_tab"), //NON-NLS
      maxHeapConf
    );
  }
  ...
  private void converterHeapSetup(Prefs globalPrefs) {
    ...
    globalPrefs.addOption(
      Resources::get_string("Prefs.converter_tab"),  //NON-NLS
      maxHeapConf
    );
  }

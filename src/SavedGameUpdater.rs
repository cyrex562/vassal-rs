      ...
      if (!(p instanceof Stack)) {
        ...
        if (slotId != null) {
            ...
            if (path != null &&
              ...
              if (!slot.getPiece().getType().equals(p.getType())) {
                if (!(p instanceof Decorator)) {
                  GameModule.getGameModule().getChatter().show(Resources::get_string("Editor.SavedGameUpdater.basic_only", p.get_name()));
                }
                else {
          ...
          // FIXME: review error message
            ...
            GameModule.getGameModule().getChatter().show(Resources::get_string("Editor.SavedGameUpdater.unable", p.get_name(), ex.getMessage()));
          }
        }
        else {
          GameModule.getGameModule().getChatter().show(Resources::get_string("Editor.SavedGameUpdater.no_slot", p.get_name()));

...
class BSHAmbiguousName extends SimpleNode
{
    ...
    public Name get_name( NameSpace namespace )
    {
        return namespace.get_nameResolver( text );
    }
		...
		throws EvalError
    {
		try {
        	return 
				get_name( callstack.top() ).toObject(
...
//e.printStackTrace();
    ...
    {
		try {
        	return get_name( callstack.top() ).toClass();
    ...
    {
		try {
			return get_name( callstack.top() ).toLHS( callstack, interpreter );
		} catch ( UtilEvalError e ) {
			throw e.toEvalError( this, callstack );
		}
    }

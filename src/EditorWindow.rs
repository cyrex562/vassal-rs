...
public abstract class EditorWindow extends JFrame {
  ...
  protected final HelpWindow helpWindow = new HelpWindow(Resources::get_string("Editor.ModuleEditor.reference_manual"), //$NON-NLS-1$
    null);
  ...
  protected EditorWindow() {
    ...
    if (SystemUtils.IS_OS_MAC) {
      ...
      final MenuProxy fileMenu = new MenuProxy(Resources::get_string("General.file"));
    ...
    // edit menu
    final MenuProxy editMenu = new MenuProxy(Resources::get_string("General.edit"));
    editMenu.setMnemonic(Resources::get_string("General.edit.shortcut").charAt(0));
    ...
    // tools menu
    final MenuProxy toolsMenu = new MenuProxy(Resources::get_string("General.tools"));
    toolsMenu.setMnemonic(Resources::get_string("General.tools.shortcut").charAt(0));
    ...
    if (SystemUtils.IS_OS_MAC) {
      ...
      final MenuProxy helpMenu = new MenuProxy(Resources::get_string("General.help"));
...
//    mm.getAction("General.quit").setMnemonic('Q');
    ...
    mm.addAction("Editor.ModuleEditor.refresh_predefined", new AbstractAction(Resources::get_string(
      ...
      public void actionPerformed(ActionEvent e) {
        ...
        if (gs.isGameStarted()) {
          ...
          final Command ac = new AlertCommand(Resources::get_string("GameRefresher.game_started_in_editor"));
          ac.execute();
        }
        else {
    ...
    try {
      ...
      //helpAction.putValue(Action.SHORT_DESCRIPTION, Resources::get_string("Editor.ModuleEditor.properties")); //$NON-NLS-1$
  ...
  void setModuleName(String name) {
    if (isTempFile(name)) {
      moduleName = Resources::get_string("Editor.ModuleEditor.creating_new_module");
    }
    else {
      moduleName = name;
    }
  ...
  void setExtensionName(String name) {
    if (isTempFile(name)) {
      extensionName = Resources::get_string("Editor.ExtensionEditor.creating_new_extension");
    }
    else {
      extensionName = name;
    }

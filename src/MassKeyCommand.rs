...
public class MassKeyCommand extends AbstractToolbarItem
  ...
  public MassKeyCommand() {
    ...
    setLaunchButton(makeLaunchButton(
      Resources::get_string("Editor.GlobalKeyCommand.button_name"),
      Resources::get_string("Editor.GlobalKeyCommand.button_name"),
  ...
  public void apply() {
    ...
    final AuditTrail audit = AuditTrail.create(this, "", Resources::get_string("Editor.MassKey.match"));
    if (singleMap) {
  ...
  public String[] getAttributeDescriptions() {
    if (condition == null) {
      return ArrayUtils.addAll(
        ...
        Resources::get_string("Editor.GlobalKeyCommand.global_key_command"), //$NON-NLS-1$       // Key Command
        Resources::get_string("Editor.MassKey.counters"), //$NON-NLS-1$       // Apply to counters on this map only

        Resources::get_string("Editor.GlobalKeyCommand.pre_select"),          // Fast match target info

        Resources::get_string("Editor.MassKey.match"), //$NON-NLS-1$          // Match properties        
        Resources::get_string("Editor.GlobalKeyCommand.deck_policy"), //$NON-NLS-1$   // Apply to pieces in deck
        Resources::get_string("Editor.MassKey.suppress"), //$NON-NLS-1$       // Suppress individual reports?
        Resources::get_string("Editor.report_format") //$NON-NLS-1$           // Report format
      );
    }
    else {
      ...
      return ArrayUtils.addAll(
        super.getAttributeDescriptions(),
        Resources::get_string("Editor.MassKey.key"), //$NON-NLS-1$             // Key Command
        Resources::get_string("Editor.MassKey.counters"), //$NON-NLS-1$        // Apply to counters on this map only

        Resources::get_string("Editor.GlobalKeyCommand.pre_select"),           // Fast match target info

        Resources::get_string("Editor.MassKey.match"), //$NON-NLS-1$           // Match properties
        Resources::get_string("Editor.MassKey.deck_content"), //$NON-NLS-1$    // Apply to pieces in deck
        Resources::get_string("Editor.MassKey.suppress"), //$NON-NLS-1$       // Suppress individual reports?
        Resources::get_string("Editor.report_format"), //$NON-NLS-1$          // Report format
        Resources::get_string("Editor.MassKey.apply") //$NON-NLS-1$           // Legacy condition
      );
    }
  }
  ...
  public static class DeckPolicyConfig extends Configurer implements ConfigurerFactory, Auditable {
    ...
    public DeckPolicyConfig(boolean showPrompt) {
      ...
      if (showPrompt) {
        ...
        prompt = new JLabel(Resources::get_string("Editor.GlobalKeyCommand.deck_policy"));
  ...
  public static String getConfigureTypeName() {
    return Resources::get_string("Editor.GlobalkeyCommand.global_key_command");
  }
      ...
      if (names.length == 0) {
        ...
        filter = piece -> {
          for (final String s : names) {
            if (Decorator.getInnermost(piece).get_name().equals(s)) {
              return true;
            }
          }
          return false;

...
public class SimpleStatusControlsInitializer implements ChatControlsInitializer {
  ...
  public void initializeControls(final ChatServerControls controls) {
    ...
    if (includeLooking) {
      lookingBox = new JButton(Resources::get_string("Chat.looking_for_a_game")); //$NON-NLS-1$
      lookingBox.addActionListener(evt -> {
        if ((client != null) && client.isConnected()) {
          ...
          client.setUserInfo(new SimplePlayer(p.getId(), p.get_name(), s));

          if (s.isLooking()) {
            GameModule.getGameModule().warn(Resources::get_string("Chat.now_looking"));
          }
          else {
            GameModule.getGameModule().warn(Resources::get_string("Chat.not_looking"));
          }
        }
        else {
          GameModule.getGameModule().warn(Resources::get_string("Chat.connect_first_looking"));
        }
      });
    ...
    awayButton = new JButton(Resources::get_string("Chat.away_from_keyboard")); //$NON-NLS-1$
    awayButton.addActionListener(evt -> {
      if (client != null) {
        ...
        client.setUserInfo(new SimplePlayer(p.getId(), p.get_name(), s));
        JOptionPane.showMessageDialog(controls.getRoomTree(), Resources::get_string("Chat.im_back"), Resources::get_string("Chat.away_from_keyboard"), JOptionPane.PLAIN_MESSAGE); //$NON-NLS-1$ //$NON-NLS-2$

...
public abstract class AbstractBuildable extends AbstractImageFinder implements Buildable, ValidityChecker, PropertyNameSource, Auditable {
  ...
  public void build(Element e) {
    ...
    for (int i = 0; i < n.getLength(); ++i) {
      ...
      setAttribute(att.get_name(), att.getValue().intern());
      ...
      if (this instanceof Translatable) {
        Localization.getInstance().saveTranslatableAttribute((Translatable) this, att.get_name(), att.getValue());
      }
    }
   ...
   * @deprecated Use {@link #get_components_of(Class)} instead.
   */
  @Deprecated(since = "2020-08-06", forRemoval = true)
  public <T> Enumeration<T> getComponents(Class<T> target) {
    return Collections.enumeration(get_components_of(target));
  }

  /**
   * @return all build components that are an instance of the given class
   */
  public <T> List<T> get_components_of(Class<T> target) {
    ...
    for (final Buildable b : buildComponents) {
      if (target.isInstance(b)) {
        ...
        l.addAll(((AbstractFolder)b).get_components_of(target));
      }
    }
    return l;
  }

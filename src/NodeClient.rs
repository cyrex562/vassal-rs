...
public class NodeClient implements LockableChatServerConnection,
    ...
    if (connect) {
      if (!isConnected()) {
        ...
        // FIXME: review error message
          ...
          propSupport.firePropertyChange(STATUS, null, Resources::get_string(
  ...
  public void socketClosed(SocketHandler handler) {
    ...
    synchronized (this) {
      if (sender != null) {
        propSupport.firePropertyChange(STATUS, null, Resources::get_string("Server.lost_connection")); //$NON-NLS-1$
  ...
  public boolean isDefaultRoom(Room r) {
    return r != null && r.get_name().equals(getDefaultRoomName());
  }
  ...
  public void forward(String receipientPath, String msg) {
    if (isConnected() && currentRoom != null && msg != null) {
      msg = checker.filter(msg, defaultRoomName, currentRoom.get_name());
  ...
  public void sendToOthers(String msg) {
    if (currentRoom != null) {
      final String path = new SequenceEncoder(moduleName, '/').append(
          currentRoom.get_name()).append("~" + me.getId()).getValue(); //$NON-NLS-1$
      forward(path, msg);
    }
  }
  ...
  public void sendInvite(Player invitee) {
    sendTo(invitee, new InviteCommand(me.get_name(), me.getId(), getRoom()
        .get_name()));
  }
  ...
  public void sendRoomInfo(NodeRoom r) {
    final Node dummy = new Node(null, r.get_name(), new PropertiesEncoder(r.getInfo())
        .getStringValue());
    if (isConnected()) {
  ...
  public void setRoom(Room r, String password) {
    if (isConnected()) {
      final String newRoom = r.get_name();
      ...
      if (r instanceof NodeRoom) {
        ...
        if (newRoom.equals(defaultRoomName)) {
          ...
          GameModule.getGameModule().warn(
              Resources::get_string("Chat.synchronize_pending"));
        }
      }
    }
  }
  ...
  public void handleMessageFromServer(String msg) {
    ...
    if ((n = Protocol.decodeListCommand(msg)) != null) {
      ...
      if (pendingSynchToRoom != null) {
        ...
        GameModule.getGameModule().warn(Resources::get_string("Chat.synchronize_complete"));

        final GameModule gm = GameModule.getGameModule();
        final Chatter chatter = gm.getChatter();
        final String playerName = getUserInfo().get_name();
        final List<String> errors = new ArrayList<>();

        final boolean compatible = checkCompatibility(pendingSynchToRoom, errors);

        Command chat = new Chatter.DisplayText(chatter, "-!<b> &lt;" + playerName + "&gt; " + Resources::get_string("Chat.joining_room_chat", pendingSynchToRoom.get_name()));
        ...
        chat = chat.append(new Chatter.DisplayText(chatter, compatible ? "-!<b> " + Resources::get_string("Chat.join_ok", playerName)  : "-!<b> " + Resources::get_string("Chat.join_not_ok", playerName)));
      ...
      for (final NodeRoom aRoom : allRooms) {
        final String infoString = p.getProperty(aRoom.get_name());
  ...
  protected void updateRooms(Node module) {
    ...
    for (int i = 0; i < roomNodes.length; ++i) {
      ...
      if (!rooms[i].get_name().equals(defaultRoomName)) {
        rooms[i].lock();
      }
      ...
      if (defaultRoomName.equals(rooms[i].get_name())) {
        defaultRoomIndex = i;
      }
    }
  ...
  public void initializeControls(ChatServerControls controls) {
    ...
    s = new SimpleStatus(s.isLooking(), s.isAway(), (String) g.getPrefs()
        .getValue(GameModule.PERSONAL_INFO), Info.get_version(), s.getIp(), g
        .get_game_version()
        + ((g.getArchiveWriter() == null) ? "" : " " + Resources::get_string("Editor.NodeClient.editing")),
  ...
  public boolean checkCompatibility(NodeRoom targetRoom, List<String> errors) {
    ...
    if (defaultRoomName.equals(targetRoom.get_name()) || targetRoom.getOwningPlayer() == null) {
      return compatible;
    }
    ...
    if (!VersionUtils.truncateToMinorVersion(ownerStatus.getClient()).equals(VersionUtils.truncateToMinorVersion(myStatus.getClient()))) {
      errors.add(Resources::get_string("Chat.bad_vassal", myStatus.getClient(), ownerStatus.getClient()));
      compatible = false;
    }
    ...
    if (!ownerVersion.equals(myVersion)) {
      errors.add(Resources::get_string("Chat.bad_module", myVersion, ownerVersion));
      compatible = false;
    }
  ...
  public static String cleanVersion(String version) {
    final String editingString = " " + Resources::get_string("Editor.NodeClient.editing");

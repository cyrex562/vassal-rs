...
public class JoinRoomAction extends AbstractAction {
  ...
  public JoinRoomAction(Room r, ChatServerConnection client) {
    super(Resources::get_string("Chat.join_room")); //$NON-NLS-1$
    this.r = r;
    this.client = client;
  ...
  public static void explainMainRoom() {
    GameModule.getGameModule().warn(Resources::get_string("Chat.explain_main_room"));
  }
  ...
  public void actionPerformed(ActionEvent evt) {
    if (client instanceof NodeClient) {
      ...
      if (!compatible) {
        ...
        sb.append(Resources::get_string("Chat.join_issue", r.get_name())).append("\n\n");
        for (final String error : errors) {
          sb.append(" - ").append(error).append('\n');
        }
        sb.append('\n').append(Resources::get_string("Chat.are_you_sure"));
        Dialogs.showMessageDialog(GameModule.getGameModule().getPlayerWindow(),
          Resources::get_string("Chat.vassal_configuration_error"),
          Resources::get_string("Chat.vassal_configuration_error"),
          sb.toString(),
    ...
    if (r != null) {
      ...
      gm.warn(Resources::get_string("Chat.joining_room", r.get_name()));

      if (Resources::get_string("Chat.main_room").equals(r.get_name())) {
        explainMainRoom();
      }
      else {
        gm.warn(Resources::get_string("Chat.explain_joined_room"));
      }
    }
  }

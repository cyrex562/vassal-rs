...
public class ExpressionBuilder extends JDialog {
  ...
  public ExpressionBuilder(Configurer c, JDialog parent, EditablePiece piece) {
    super(parent, Resources::get_string("Editor.ExpressionBuilder.component_type"), true);
    target = c;
    pieceTarget = piece;
  ...
  public void setExpression(String value) {
    if (expression == null) {
      final String prompt = target.get_name().length() == 0 ? Resources::get_string("Editor.ExpressionBuilder.expression") : target.get_name();

...
public class AutoConfigurer extends Configurer
  ...
  public AutoConfigurer(AutoConfigurable c) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      if (config != null) {
        ...
        if (Resources::get_string("Editor.menu_command").equals(prompt[i])) {
          config.setHintKey("Editor.menu_command_hint");
        }
        else if (Resources::get_string("Editor.description_label").equals(prompt[i])) {
          config.setHintKey("Editor.description_component_hint");
        }
        else if (Resources::get_string("Editor.report_format").equals(prompt[i])) {
          config.setHintKey("Editor.report_format_hint");
        }
        else if (Resources::get_string("Editor.button_text_label").equals(prompt[i])) {
          config.setHintKey("Editor.button_text_hint");
        }
        else if (Resources::get_string("Editor.tooltip_text_label").equals(prompt[i])) {
  ...
  public static Configurer createConfigurer(Class<?> type,
    ...
    if (String.class.isAssignableFrom(type)) {
      ...
      throw new IllegalArgumentException("Invalid class " + type.get_name());
    }
    return config;
  }

...
public class BasicLogger implements Logger, Buildable, GameComponent, CommandEncoder {
   ...
   */
  public List<NamedKeyStroke> get_namedKeyStrokes() {
  ...
  public void addTo(Buildable b) {
    ...
    newLogAction.putValue(Action.MNEMONIC_KEY, (int)Resources::get_string("BasicLogger.begin_logfile.shortcut").charAt(0)); // Separate shortcut key thus possible for each language
    mm.addAction("BasicLogger.begin_logfile", newLogAction); //NON-NLS
    endLogAction.putValue(Action.MNEMONIC_KEY, (int)Resources::get_string("BasicLogger.end_logfile.shortcut").charAt(0)); // Separate shortcut key thus possible for each language
    ...
    button.setToolTipText(Resources::get_string("BasicLogger.undo_last_move"));  //$NON-NLS-1$
    button.setAlignmentY((float) 0.0);

    button = mod.getToolBar().add(stepAction);
    button.setToolTipText(Resources::get_string("BasicLogger.step_forward_tooltip"));  //$NON-NLS-1$
    button.setAlignmentY((float) 0.0);
    ...
    final IconConfigurer stepIconConfig = new IconConfigurer("stepIcon", Resources::get_string("BasicLogger.step_forward_button"), STEP_ICON); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    stepKeyConfig = new NamedHotKeyConfigurer("stepHotKey", Resources::get_string("BasicLogger.step_forward_hotkey"), stepKeyListener.get_namedKeyStroke());  //$NON-NLS-1$ //$NON-NLS-2$
    ...
    stepKeyConfig.addPropertyChangeListener(evt -> {
      ...
      if (stepKeyListener.getKeyStroke() != null) {
        stepAction.putValue(Action.SHORT_DESCRIPTION, Resources::get_string("BasicLogger.step_forward_tooltip2", NamedHotKeyConfigurer.getString(stepKeyListener.getKeyStroke())));  //$NON-NLS-1$
      }
      else {
        stepAction.putValue(Action.SHORT_DESCRIPTION, Resources::get_string("BasicLogger.step_forward_tooltip3"));  //$NON-NLS-1$
      }
    });
    stepKeyConfig.fireUpdate();

    final IconConfigurer undoIconConfig = new IconConfigurer("undoIcon", Resources::get_string("BasicLogger.undo_icon"), UNDO_ICON);  //$NON-NLS-1$ //$NON-NLS-2$
    ...
    undoKeyConfig = new NamedHotKeyConfigurer("undoHotKey", Resources::get_string("BasicLogger.undo_hotkey"), undoKeyListener.get_namedKeyStroke()); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    undoKeyConfig.addPropertyChangeListener(evt -> {
      ...
      if (undoKeyListener.getKeyStroke() != null) {
        undoAction.putValue(Action.SHORT_DESCRIPTION, Resources::get_string("BasicLogger.undo_tooltip2", NamedHotKeyConfigurer.getString(undoKeyListener.getKeyStroke()))); //$NON-NLS-1$
      }
      else {
        undoAction.putValue(Action.SHORT_DESCRIPTION, Resources::get_string("BasicLogger.undo_last_move")); //$NON-NLS-1$
      }
    });
    undoKeyConfig.fireUpdate();

    final BooleanConfigurer logOptionStart = new BooleanConfigurer(PROMPT_NEW_LOG_START, Resources::get_string("BasicLogger.prompt_new_log_before"), Boolean.TRUE);  //$NON-NLS-1$
    mod.getPrefs().addOption(Resources::get_string("Prefs.general_tab"), logOptionStart); //$NON-NLS-1$

    final BooleanConfigurer logOptionEnd = new BooleanConfigurer(PROMPT_NEW_LOG_END, Resources::get_string("BasicLogger.prompt_new_log_after"), Boolean.TRUE);  //$NON-NLS-1$
    mod.getPrefs().addOption(Resources::get_string("Prefs.general_tab"), logOptionEnd); //$NON-NLS-1$

    final BooleanConfigurer logOptionComment = new BooleanConfigurer(PROMPT_LOG_COMMENT, Resources::get_string("BasicLogger.enable_comments"), Boolean.TRUE);  //$NON-NLS-1$
    mod.getPrefs().addOption(Resources::get_string("Prefs.general_tab"), logOptionComment); //$NON-NLS-1$
  }

  @Override
  public Element getBuildElement(Document doc) {
    return doc.createElement(getClass().get_name());
  }
    ...
    if (startingGame) {
      ...
      if (endLogAction.isEnabled() && (beginningState != null)) {
        if (JOptionPane.showConfirmDialog(
            ...
            Resources::get_string("BasicLogger.save_log"),    //$NON-NLS-1$
            Resources::get_string("BasicLogger.unsaved_log"), //$NON-NLS-1$
  ...
  public void queryNewLogFile(boolean atStart, boolean fastForwarded) {
    ...
    if (atStart) {
      ...
      prompt = fastForwarded ? Resources::get_string("BasicLogger.append_commencing") : Resources::get_string("BasicLogger.replay_commencing");
    }
    else {
      prefName = PROMPT_NEW_LOG_END;
      prompt = fastForwarded ? Resources::get_string("BasicLogger.fast_forward_completed") : Resources::get_string("BasicLogger.replay_completed");
    }

    if (Boolean.TRUE.equals(g.getPrefs().getValue(prefName)) || (atStart && fastForwarded)) {
      final Object[] options = {
        Resources::get_string(Resources.YES),
        Resources::get_string(Resources.NO),
        Resources::get_string("BasicLogger.dont_prompt_again")  //$NON-NLS-1$
      };

      final int result = JOptionPane.showOptionDialog(
        g.getPlayerWindow(),
        Resources::get_string("BasicLogger.start_new_log_file", prompt), //$NON-NLS-1$
        "",  //$NON-NLS-1$
...
// FIXME: Extremely inefficient! Make encode write to an OutputStream
  ...
  private File getSaveFile() {
    ...
    String name = fc.getSelectedFile() == null
      ? null : fc.getSelectedFile().get_name();
    if (name != null) {
    ...
    if (!file.get_name().endsWith(".vlog")) { //NON-NLS
      file = new File(file.getParent(), file.get_name() + ".vlog"); //NON-NLS
    }
      ...
      if (md instanceof SaveMetaData) {
        if (Info.hasOldFormat(md.getVassalVersion())) {
          ...
          final int result = Dialogs.showConfirmDialog(
            g.getPlayerWindow(),
            Resources::get_string("Warning.log_will_be_updated_title"),
            Resources::get_string("Warning.log_will_be_updated_heading"),
  ...
  protected void beginOutput() {
    ...
    gm.setGameFile(outputFile.get_name(), GameModule.GameFileMode.LOGGING_GAME);
    GameModule.getGameModule().warn(Resources::get_string("BasicLogger.logging_begun"));  //$NON-NLS-1$
    newLogAction.setEnabled(false);
  ...
  protected Action endLogAction = new AbstractAction(Resources::get_string("BasicLogger.end_logfile")) {  //$NON-NLS-1$
    ...
    public void actionPerformed(ActionEvent e) {
      try {
        if (beginningState != null) {
          write();
          GameModule.getGameModule().warn(Resources::get_string("BasicLogger.logfile_written"));  //$NON-NLS-1$
  ...
  protected Action newLogAction = new AbstractAction(Resources::get_string("BasicLogger.begin_logfile")) {  //$NON-NLS-1$
    private static final long serialVersionUID = 1L;
    ...
    public LogCommand(Command c, List<Command> logInput, Action stepAction) {
      if (c instanceof LogCommand) {
        throw new UnsupportedOperationException(
          Resources::get_string("BasicLogger.cant_log"));  //$NON-NLS-1$
      }
  ...
  public class StepAction extends AbstractAction {
    ...
    public StepAction() {
      ...
      if (iconURL != null) {
        ...
        putValue(Action.NAME, Resources::get_string("BasicLogger.step"));  //$NON-NLS-1$
      }
    }
  ...
  public class UndoAction extends AbstractAction {
    ...
    public UndoAction() {
      ...
      if (iconURL != null) {
        ...
        putValue(Action.NAME, Resources::get_string("BasicLogger.undo"));  //$NON-NLS-1$
      }
    }

...
public class P2PClient implements ChatServerConnection, ChatControlsInitializer, UserDialog, PlayerEncoder {
  ...
  public void setRoom(Room r) {
    if (me instanceof P2PPlayer) {
      ((P2PPlayer) me).setRoom(r.get_name());
      if (isConnected()) {
  ...
  public void setConnected(boolean connect) {
    if (connect) {
      try {
        ...
        p.updateStatus();
        p.setName(me.get_name());
        p.setRoom(roomMgr.getDefaultRoom().get_name());
        ...
        fireStatus(Resources::get_string("Peer2Peer.server_connection_established", params.getProperty(P2PClientFactory.P2P_LISTEN_PORT))); //$NON-NLS-1$
      ...
      // FIXME: review error message
        ...
        fireStatus(Resources::get_string("Peer2Peer.connection_error", e.getMessage())); //$NON-NLS-1$
        fireStatus(Resources::get_string("Peer2Peer.disconnected")); //$NON-NLS-1$ //$NON-NLS-2$
        connected = false;
  ...
  public synchronized void showStreamsFailed(PeerInfo pPeerInfo) {
    ...
    propSupport.firePropertyChange(STATUS, null, Resources::get_string("Peer2Peer.connection_lost", p.get_name())); //$NON-NLS-1$
  }
  ...
  public synchronized void showHELO(PeerInfo pPeerInfo) {
    ...
    if (! pPeerInfo.getNetworkPw().equals(params.getProperty(P2PClientFactory.P2P_SERVER_PW))) {
      new Chatter.DisplayText(chatter, Resources::get_string("Peer2Peer.bad_password", details)).execute();
      peer.finish();
      return;
    }

    fireStatus(Resources::get_string("Peer2Peer.connected", details));

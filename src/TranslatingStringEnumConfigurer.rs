...
public class TranslatingStringEnumConfigurer extends Configurer {
   ...
   * @param validValues Array of values to maintain
   * @param i18nKeys Array of Translation Keys used to describe the list of values
   * @param isDisplayNames True if we have already been given full translated strings; false if they are "keys" for Resources::get_string()
   */
  ...
  public Component getControls() {
    if (panel == null) {
      ...
      for (int i = 0; i < i18nKeys.length; i++) {
        displayValues[i] = isDisplayNames() ? i18nKeys[i] : Resources::get_string(i18nKeys[i]);
      }

      panel = new ConfigurerPanel(get_name(), "[]", "[][]");

...
public class MapBoard extends Importer {
  ...
  protected class MapLayer {
    ...
    void writeToArchive() throws IOException {
      ...
      if (imageName != null && r != null && r.width > 0 && r.height > 0) {
        ...
        stack.setAttribute(SetupStack.NAME, get_name());
        ...
        if (isSwitchable()) {
          ...
          control.setAttribute(LayerControl.BUTTON_TEXT, get_name());
          control.setAttribute(LayerControl.TOOLTIP, "Toggle " + get_name().toLowerCase() + " visibility");
          control.setAttribute(LayerControl.COMMAND, LayerControl.CMD_TOGGLE);
          control.setAttribute(LayerControl.LAYERS, get_name());
          ...
          if (entries.equals("")) {
            ...
            entries = entries + "," + new SequenceEncoder(get_name(), ',').getValue();
          }
    ...
    protected Rectangle writeImageToArchive() throws IOException {
      ...
      try {
        ...
        imageName = getUniqueImageFileName(get_name(), ".png");
  ...
  protected class MapSheet {
    ...
    Zone getZone() {
      if (zone == null) {
        zone = new Zone();
        zone.setConfigureName(get_name());
...
//      assert (size > 0);
  ...
  protected void load(File f) throws IOException {
    ...
    try (InputStream fin = Files.newInputStream(f.toPath());
         InputStream bin = new BufferedInputStream(fin);
         DataInputStream in = new DataInputStream(bin)) {
      baseName = stripExtension(f.get_name());
      path = f.getPath();
  ...
  private ToolbarMenu getToolbarMenu() {
    final List<ToolbarMenu> list = getMainMap().get_components_of(ToolbarMenu.class);

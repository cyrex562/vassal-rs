...
public class ServerAddressBook {
  ...
  private static String discoverMyIpAddressFromRemote() throws IOException {
    ...
    if (!l.isEmpty()) {
      ...
      throw new IOException(Resources::get_string("Server.empty_response")); //$NON-NLS-1$
    }
    return theIp;
  }
  ...
  public JComponent getControls() {
    if (controls == null) {
      ...
      setButton = new JButton(Resources::get_string("ServerAddressBook.set_current")); //$NON-NLS-1$
      setButton.setToolTipText(Resources::get_string("ServerAddressBook.set_selected_server")); //$NON-NLS-1$
      setButton.addActionListener(e -> setCurrentServer(myList.getSelectedIndex()));

      addButton = new JButton(Resources::get_string(Resources.ADD));
      addButton.setToolTipText(Resources::get_string("ServerAddressBook.add_server")); //$NON-NLS-1$
      addButton.addActionListener(e -> addServer());

      removeButton = new JButton(Resources::get_string(Resources.REMOVE));
      removeButton.setToolTipText(Resources::get_string("ServerAddressBook.remove_selected_server")); //$NON-NLS-1$
      removeButton.addActionListener(e -> removeServer(myList.getSelectedIndex()));

      editButton = new JButton(Resources::get_string(Resources.EDIT));
      editButton.setToolTipText(Resources::get_string("ServerAddressBook.edit_server")); //$NON-NLS-1$
  ...
  private void removeServer(int index) {
    ...
    final int i = JOptionPane.showConfirmDialog(
      ...
      Resources::get_string("ServerAddressBook.remove_server", e.getDescription()) //$NON-NLS-1$
    );
    if (i == 0) {
  ...
  private void addServer() {
    ...
    final JMenuItem p2pItem = new JMenuItem(Resources::get_string("ServerAddressBook.peer_server"));
    p2pItem.addActionListener(e -> addEntry(new PeerServerEntry()));

    final JMenuItem privateItem = new JMenuItem(Resources::get_string("ServerAddressBook.private_server"));
    ...
    public boolean edit(boolean enabled) {
      if (isEditable()) {
        ...
        final Integer result = (Integer) Dialogs.showDialog(null,
            Resources::get_string("ServerAddressBook.edit_server_configuration"), //$NON-NLS-1$
    ...
    private class ServerConfig {
      ...
      public JComponent getControls() {
        if (configControls == null) {
          ...
          configPanel.add(new JLabel(Resources::get_string("Editor.description_label"))); //$NON-NLS-1$
  ...
  private class OfficialEntry extends AddressBookEntry {
    public OfficialEntry() {
      this(new Properties());
      setDescription(Resources::get_string("ServerAddressBook.official_server")); //$NON-NLS-1$
  ...
  private class PrivateEntry extends AddressBookEntry {
    ...
    public PrivateEntry() {
      this(new Properties());
      setDescription(Resources::get_string("ServerAddressBook.private_server")); //$NON-NLS-1$
    ...
    @Override
    public String toString() {
      return Resources::get_string("ServerAddressBook.private_server") + " [" + getDescription() + "]";
    }
    ...
    protected void addAdditionalControls(JComponent c, boolean enabled) {
      ...
      c.add(new JLabel(Resources::get_string("ServerAddressBook.server_ip"))); //$NON-NLS-1$
      c.add(serverIp, "wrap, growx, push"); //$NON-NLS-1$

      serverPort.setEditable(enabled);
      c.add(new JLabel(Resources::get_string("ServerAddressBook.server_port"))); //$NON-NLS-1$
  ...
  private class PeerServerEntry extends AddressBookEntry {
    ...
    public PeerServerEntry() {
      super();
      setDescription(Resources::get_string("ServerAddressBook.peer_server")); //$NON-NLS-1$
      setType(P2P_TYPE);
    ...
    @Override
    public String toString() {
      return Resources::get_string("ServerAddressBook.peer_server") + " [" + getProperty(DESCRIPTION_KEY) + "]";
    }
    ...
    protected void addAdditionalControls(JComponent c, boolean enabled) {
      ...
      c.add(new JLabel(Resources::get_string("ServerAddressBook.listen_port"))); //$NON-NLS-1$
      c.add(listenPort, "wrap, growx, push"); //$NON-NLS-1$

      serverPw.setEditable(enabled);
      c.add(new JLabel(Resources::get_string("ServerAddressBook.server_password"))); //$NON-NLS-1$
      c.add(serverPw, "wrap, growx, push"); //$NON-NLS-1$

      c.add(new JLabel(Resources::get_string("Peer2Peer.internet_address"))); //$NON-NLS-1$
      ...
      if (!getLocalAddress().equals(getExternalAddress())) {
        c.add(new JLabel(Resources::get_string("Peer2Peer.local_address"))); //$NON-NLS-1$
    ...
    public Component getListCellRendererComponent(JList list, Object value, int index,
      ...
      if (value instanceof AddressBookEntry) {
        ...
        if (e.isCurrent()) {
          ...
          setText(e + Resources::get_string("ServerAddressBook.current")); //$NON-NLS-1$
        }
        else {

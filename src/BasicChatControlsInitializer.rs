...
public class BasicChatControlsInitializer implements ChatControlsInitializer {
  ...
  public void initializeControls(final ChatServerControls controls) {
    ...
    connectAction = new AbstractAction(
                          Resources::get_string("Chat.connect")) {  //$NON-NLS-1$
      private static final long serialVersionUID = 1L;

      @Override
      public void actionPerformed(ActionEvent evt) {
        GameModule.getGameModule().warn(Resources::get_string("Chat.connecting"));
    ...
    disconnectAction = new AbstractAction(
                        Resources::get_string("Chat.disconnect")) {  //$NON-NLS-1$
      private static final long serialVersionUID = 1L;
    ...
    connectionListener = evt -> SwingUtilities.invokeLater(() -> {
      ...
      if (!connected) {
        ...
        GameModule.getGameModule().warn(Resources::get_string("Chat.disconnected")); 
      }
      else {
        GameModule.getGameModule().warn(Resources::get_string("Chat.joining_main_room"));

  ...
  private void internalExtractContents(ZipInputStream in) throws IOException {
    ...
    while ((entry = in.getNextEntry()) != null) {
      if (entry.isDirectory()) {
        Files.createDirectories(p.resolve(entry.get_name()));
      }
      else {
// FIXME: no way to distinguish between read and write errors here
        try (OutputStream fos = Files.newOutputStream(p.resolve(entry.get_name()))) {
          in.transferTo(fos);
        }
      }
    }
  ...
  public static String getConfigureTypeName() {
    return Resources::get_string("Editor.BrowserHelpFile.component_type"); //$NON-NLS-1$
  }
  ...
  protected class ConfigSupport implements AutoConfigurable {
    ...
    public String[] getAttributeDescriptions() {
      return new String[]{
        ...
        Resources::get_string("Editor.BrowserHelpFile.contents"),
        Resources::get_string("Editor.BrowserHelpFile.starting_page")
      };
    }
    ...
    protected void packFile(File packed, String prefix, ZipOutputStream out)
      ...
      if (packed.isDirectory()) {
        for (final File f : packed.listFiles()) {
          packFile(f, prefix + packed.get_name() + "/", out); //$NON-NLS-1$
        }
      }
      else {
        final ZipEntry entry = new ZipEntry(prefix + packed.get_name());
  ...
  public ComponentI18nData getI18nData() {
    if (myI18nData == null) {
      myI18nData = new ComponentI18nData(this, "BrowserHelpFile." + getConfigureName(), null, //NON-NLS
          new String[] {TITLE},
          new boolean[] {true},
          new String[] {Resources::get_string("Editor.menu_command")});
    }
    return myI18nData;
  }
}

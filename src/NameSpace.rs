	...
	NameSource
{
	...
	*/
		...
		throws UtilEvalError 
	{
		...
		return get_nameResolver( name ).toObject( callstack, interpreter );
	}
...
// this?
			...
			*/
			...
			if ( clas == null ) 
			{
				...
				if ( Name.isCompound( fullname ) )
					try {
						clas = get_nameResolver( fullname ).toClass();
	...
	*/
	...
	/**
		...
		(This method would be called get_name() if it weren't already used for
		...
		Use get() instead.
	*/
	Name get_nameResolver( String ambigname )
	{
		if ( names == null )
			names = new Hashtable();

    ...
    for (final LoadConstraint rule : constraints) {
      if (rule.moduleName.equals(name)) {
        if (!rule.minVassalVersion.isEmpty() && (VersionUtils.compareVersions(md.getVassalVersion(), rule.minVassalVersion) < 0)) {
          ErrorDialog.show("Error.module_too_old_vassal_version", name, Info.get_version(), rule.newVersionLink); //NON-NLS
          return false;
        }
        if (!rule.minModuleVersion.isEmpty() && (VersionUtils.compareVersions(md.get_version(), rule.minModuleVersion) < 0)) {
          ErrorDialog.show("Error.module_too_old_module_version", name, Info.get_version(), rule.newVersionLink);  //NON-NLS
          return false;
        }
      }
    }

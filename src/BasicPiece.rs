...
public class BasicPiece extends AbstractImageFinder implements TranslatablePiece, StateMergeable, PropertyNameSource, PersistentPropertyContainer,
  ...
  public Object getPublicProperty(Object key) {
    if (Properties.KEY_COMMANDS.equals(key)) {
      ...
      return Decorator.getOutermost(this).get_name();
    }
  ...
  protected KeyCommand[] getKeyCommands() {
    if (commands == null) {
      ...
      if (cloneKey > 0) {
        l.add(new KeyCommand(Resources::get_string("Editor.Clone.clone"), KeyStroke.getKeyStroke(cloneKey, InputEvent.CTRL_DOWN_MASK), target));
      }
      if (deleteKey > 0) {
        l.add(new KeyCommand(Resources::get_string("Editor.Delete.delete"), KeyStroke.getKeyStroke(deleteKey, InputEvent.CTRL_DOWN_MASK), target));
      }
    ...
    enableCommand(Resources::get_string("Editor.Clone.clone"), outer.getMap() != null);
    enableCommand(Resources::get_string("Editor.Delete.delete"), outer.getMap() != null);
    return commands;
  }
  ...
  private void enableCommand(String name, boolean enable) {
    for (final KeyCommand command : commands) {
      if (name.equals(command.get_name())) {
  ...
  public String getRealLocalizedName() {
    final String key = TranslatablePiece.PREFIX + get_name();
    return Localization.getInstance().translate(key, get_name());
  }
    ...
    if (cloneKey != 0 && KeyStroke.getKeyStroke(cloneKey, InputEvent.CTRL_DOWN_MASK).equals(stroke)) {
      ...
      if (getMap() != null) {
        ...
        if (GlobalOptions.getInstance().autoReportEnabled() && !Boolean.TRUE.equals(outer.getProperty(Properties.INVISIBLE_TO_OTHERS))) {
          ...
          final String s;
          if (loc != null) {
            s = Resources::get_string("BasicPiece.clone_report_1", name, loc);
          }
          else {
            s = Resources::get_string("BasicPiece.clone_report_2", name);
          }
      ...
      if (getMap() != null && GlobalOptions.getInstance().autoReportEnabled() && !Boolean.TRUE.equals(outer.getProperty(Properties.INVISIBLE_TO_OTHERS))) {
        ...
        final String s;
        if (loc != null) {
          s = Resources::get_string("BasicPiece.delete_report_1", name, loc);
        }
        else {
          s = Resources::get_string("BasicPiece.delete_report_2", name);
        }
        final Command report = new Chatter.DisplayText(GameModule.getGameModule().getChatter(), s);
        comm = comm.append(report);
      }
  ...
  public void setState(String s) {
    ...
    if (!"null".equals(mapId)) { // NON-NLS
      ...
      if (newMap == null) {
        Decorator.reportDataError(this, Resources::get_string("Error.not_found", "Map"), "mapId=" + mapId); // NON-NLS
      }
    }
  ...
  public String getBaseDescription() {
    return Resources::get_string("Editor.BasicPiece.trait_description");
  }
  ...
  private static class Ed implements PieceEditor {
    ...
    private void initComponents(BasicPiece p) {
      ...
      if (p.cloneKey != 0) {
        panel.add(new JLabel(Resources::get_string("Editor.BasicPiece.to_clone")));
        panel.add(cloneKeyInput);
      }

      deleteKeyInput = new KeySpecifier(p.deleteKey);
      if (p.deleteKey != 0) {
        panel.add(new JLabel(Resources::get_string("Editor.BasicPiece.to_delete")));
  ...
  public String toString() {
    return super.toString() + "[name=" + get_name() + ",type=" + getType() + ",state=" + getState() + "]"; // NON-NLS
  }
  ...
  public PieceI18nData getI18nData() {
    ...
    data.add(commonName, Resources::get_string("Editor.BasicPiece.basic_piece_name_description"));
    return data;
  }

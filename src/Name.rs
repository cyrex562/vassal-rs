...
/**
	...
	Name objects are created by the factory method NameSpace get_nameResolver(),
...
class Name implements java.io.Serializable
{
	...
	*/
	...
	/**
		...
		Use NameSpace get_nameResolver() which supports caching.
		@see NameSpace get_nameResolver().
	*/
		...
		throws UtilEvalError
	{
		...
		*/
			...
			&& !forceClass && autoAllocateThis )
		{
			...
			Object obj = new NameSpace( 
				target_nameSpace, "auto: "+varName ).getThis( interpreter );
			target_nameSpace.setVariable( varName, obj, false );
			return completeRound( varName, suffix(evalName), obj );
		}
		...
		*/
		if ( evalBaseObject instanceof ClassIdentifier ) 
		{
			...
			// inner class?
			if ( obj == null ) {
				String iclass = clas.get_name()+"$"+field;
	...
	*/
	...
	// XXX re-eval this... do we need it?
			...
			thisNameSpace= ths.get_nameSpace();
			Object result = ths;
    ...
    */
        ...
        throws UtilEvalError, EvalError, ReflectError, InvocationTargetException
    {
		...
		if ( prefix.equals("super") && Name.countParts(value) == 2 )
		{
			...
			NameSpace thisNameSpace = ths.get_nameSpace();
			NameSpace classNameSpace = getClassNameSpace( thisNameSpace );
			if ( classNameSpace != null )
			{
        ...
        Name target_name = namespace.get_nameResolver( prefix );
        Object obj = target_name.toObject( callstack, interpreter );

		if ( obj == Primitive.VOID ) 
			throw new UtilEvalError( "Attempt to resolve method: "+methodName
					+"() on undefined variable or class name: "+target_name);
        ...
        // try static method
        if ( Interpreter.DEBUG ) 
        	Interpreter.debug("invokeMethod: trying static - " + target_name);
        ...
        // return null; ???
		throw new UtilEvalError("invokeMethod: unknown target: " + target_name);
    }

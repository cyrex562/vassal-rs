...
public class OpenContainedPiecesAction extends AbstractAction {
  ...
  public OpenContainedPiecesAction(Configurable target, HelpWindow helpWindow, Frame dialogOwner, ConfigureTree tree) {
    super(Resources::get_string("Editor.OpenContainedPiecesAction.what_it_does"));
    this.target = target;
  ...
  public void actionPerformed(ActionEvent evt) {
    ...
    if (items.length > 40) {
      final String message = Resources::get_string("Editor.OpenContainedPiecesAction.this_will_load", items.length);
      final int result = Dialogs.showConfirmDialog(null, Resources::get_string("Editor.OpenContainedPiecesAction.confirm_open"),
        Resources::get_string("Editor.OpenContainedPiecesAction.confirm_open"), message, JOptionPane.WARNING_MESSAGE,

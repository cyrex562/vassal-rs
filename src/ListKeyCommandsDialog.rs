...
public class ListKeyCommandsDialog extends JDialog {
  ...
  public ListKeyCommandsDialog(EditorWindow owner, List<Pair<String[], AbstractConfigurable>> rows) {
    super((Frame)null, Resources::get_string("Editor.ListKeyCommands.list_key_commands"), true);
  ...
  public static class CopyAction extends AbstractAction {
    ...
    public CopyAction(JTable table) {
      this.table = table;
      putValue(NAME, Resources::get_string("General.copy"));
    }
  ...
  public class JumpAction extends AbstractAction {
    ...
    public JumpAction(JTable table) {
      this.table = table;
      putValue(NAME, Resources::get_string("Editor.ListKeyCommands.jump_to_definition"));
    }
  ...
  private static class MyTableModel extends AbstractTableModel {
    ...
    public String getColumnName(int col) {
      ...
      case KEY_COMMAND_COLUMN:
        return Resources::get_string("Editor.ListKeyCommands.key_command");
      case NAMED_COMMAND_COLUMN:
        return Resources::get_string("Editor.ListKeyCommands.named_command");
      case TYPE_COLUMN:
        return Resources::get_string("Editor.ListKeyCommands.source_type");
      case NAME_COLUMN:
        return Resources::get_string("Editor.ListKeyCommands.source_name");
      case PATH_COLUMN:
        return Resources::get_string("Editor.ListKeyCommands.source_path");
      case DESC_COLUMN:
        return Resources::get_string("Editor.ListKeyCommands.source_description");
      default:
        return "";
      }
    }
  ...
  private static void checkSearchTarget(SearchTarget target, List<Pair<String[], AbstractConfigurable>> list, AbstractConfigurable configurable) {
    final List<NamedKeyStroke> keys = target.get_namedKeyStrokeList();
    if (keys != null) {
          ...
          if (!StringUtils.isEmpty(cmd_key) || !StringUtils.isEmpty(cmd_name)) {
            ...
            if (target instanceof AbstractConfigurable) {
              ...
              if (configurable instanceof PrototypeDefinition)  {
                src_name = Resources::get_string("Editor.ListKeyCommands.prototype") + ": " + configurable.getConfigureName();
              }
              else {
                src_name = ((GamePiece) target).get_name();
              }

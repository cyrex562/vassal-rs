...
public class ChessClock extends AbstractConfigurable implements CommandEncoder, GameComponent, ActionListener, ComponentDescription {
  ...
  public static final String CHESSMENU_START  = Resources::get_string("ChessClock.start"); //NON-NLS
  public static final String CHESSMENU_STOP   = Resources::get_string("ChessClock.stop"); //NON-NLS
  public static final String CHESSMENU_RESET  = Resources::get_string("ChessClock.reset"); //NON-NLS
  ...
  private class UpdateTimerCommand extends Command {
    ...
    protected void executeCommand() {
      ...
      if ((elapsedTime == -1) && (verifiedTime == -1)) {
        ...
        if (me != null) {
          GameModule.getGameModule().sendAndLog(new UpdateTimerCommand(me, get_name(), elapsedTime, verifiedTime, ticking, false));
        }
      }
  ...
  public String[] getAttributeDescriptions() {
    return new String[] {
      ...
      Resources::get_string("Editor.ChessClock.player_side_for_timer"),
      Resources::get_string("Editor.ChessClock.timer_button_text"),
      Resources::get_string("Editor.ChessClock.timer_icon"),
      Resources::get_string("Editor.ChessClock.ticking_background_color"),
      Resources::get_string("Editor.ChessClock.ticking_foreground_color"),
      Resources::get_string("Editor.ChessClock.tocking_foreground_color"),
      Resources::get_string("Editor.ChessClock.chess_clock_tooltip")
    };
  }
  ...
  */
  ...
  public static String getConfigureTypeName() {
    return Resources::get_string("Editor.ChessClock.chess_clock");
  }
  ...
  public Command getRestoreCommand() {
    ...
    return new UpdateTimerCommand(PlayerRoster.getMySide(), get_name(), elapsedTime, verified, false, true);
  }
  ...
  public Command updateState(boolean ticking) {
    ...
    if (mySide != null) {
      return new UpdateTimerCommand(mySide, get_name(), elapsedTime, verifiedTime, ticking, false);
    }
    else if (!GameModule.getGameModule().getGameState().isGameStarted()) {
      return new UpdateTimerCommand("", get_name(), elapsedTime, verifiedTime, ticking, false);
    }
    else {
  ...
  public Command resetState() {
    final String mySide = PlayerRoster.getMySide();
    if (mySide != null) {
      return new UpdateTimerCommand(mySide, get_name());
    }
    else if (!GameModule.getGameModule().getGameState().isGameStarted()) {
      return new UpdateTimerCommand("", get_name());
    }
    else {
      return new NullCommand();
    }
  ...
  private class ClockMouseListener implements MouseListener, ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      if (command.contains(CHESSMENU_START) || command.contains(CHESSMENU_STOP)) {
        ...
        c = c.append(new Chatter.DisplayText(GameModule.getGameModule().getChatter(), Resources::get_string(GlobalOptions.getInstance().chatterHTMLSupport() ? "ChessClock.reset_clock_2" : "ChessClock.reset_clock", me.playerName, side)));
      }
      c.execute();

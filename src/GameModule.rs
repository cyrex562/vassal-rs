...
public class GameModule extends AbstractConfigurable
  ...
  void build() throws IOException {
    ...
    final File f = new File(darch.get_name());
    if (!f.exists() || f.length() == 0) {
      ...
      if (!(data instanceof ModuleMetaData)) {
        throw new IOException(
          Resources::get_string("BasicModule.not_a_module"), //$NON-NLS-1$
          null);
      }
      ...
      try (InputStream inner = darch.getInputStream(properBuildFileName);
        ...
        throw new IOException(
          Resources::get_string("BasicModule.no_buildfile"), //$NON-NLS-1$
          e);
      }
      catch (IOException e) {
        throw new IOException(
          Resources::get_string("BasicModule.io_error_reading_archive"), //$NON-NLS-1$
          e);
      }
    }
  ...
  private void initIdentityPreferences() {
    ...
    final StringConfigurer fullName = new StringConfigurer(GameModule.REAL_NAME, Resources::get_string("Prefs.name_label"), Resources::get_string("Prefs.newbie"));   //$NON-NLS-1$ //$NON-NLS-2$
    fullName.addPropertyChangeListener(evt -> idChangeSupport.firePropertyChange(evt));
    final TextConfigurer profile = new TextConfigurer(GameModule.PERSONAL_INFO, Resources::get_string("Prefs.personal_info"), "");   //$NON-NLS-1$ //$NON-NLS-2$
    profile.addPropertyChangeListener(evt -> idChangeSupport.firePropertyChange(evt));
    passwordConfigurer = new ToggleablePasswordConfigurer(GameModule.SECRET_NAME, Resources::get_string("Prefs.password_label"), ""); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
    passwordConfigurer.addPropertyChangeListener(evt -> GameModule.setUserId((String) evt.getNewValue()));
    getPrefs().addOption(Resources::get_string("Prefs.personal_tab"), fullName);   //$NON-NLS-1$ //$NON-NLS-2$
    getPrefs().addOption(Resources::get_string("Prefs.personal_tab"), passwordConfigurer);   //$NON-NLS-1$ //$NON-NLS-2$
    getPrefs().addOption(Resources::get_string("Prefs.personal_tab"), profile);  //$NON-NLS-1$
  ...
  private void initServer() {
    ...
    Prefs.getGlobalPrefs().addOption(Resources::get_string("Chat.server"), config); //$NON-NLS-1$
  ...
  private void initFrame() {
    ...
    final String mess = Resources::get_string(
      "BasicModule.version_message", get_localized_game_name(), moduleVersion); //$NON-NLS-1$
    warn(mess);
    log.info(mess);
    initFrameTitle();
  }
  ...
  private void ensureComponent(Class<? extends Buildable> componentClass) {
    if (get_components_of(componentClass).isEmpty()) {
  ...
  public void setAttribute(String name, Object value) {
    if (MODULE_NAME.equals(name)) {
      ...
      final String runningVersion = Info.get_version();
  ...
  public static String getConfigureTypeName() {
    return Resources::get_string("Editor.GameModule.component_type");  //$NON-NLS-1$
  }
  ...
  public String[] getAttributeDescriptions() {
    return new String[]{
      Resources::get_string("Editor.GameModule.name_label"),    //$NON-NLS-1$
      Resources::get_string("Editor.GameModule.version_label"), //$NON-NLS-1$
      Resources::get_string("Editor.GameModule.description_label"),    //NON-NLS
      Resources::get_string("Editor.GameModule.module_other_1_label"), //NON-NLS
      Resources::get_string("Editor.GameModule.module_other_2_label") //NON-NLS
    };
  }
...
//                  InternetDiceButton.class,   // Disable internet dice button until Bones server can prevent email spamming
   ...
   */
  public String get_localized_game_name() {
   ...
   */
  public String get_game_version() {
    return moduleVersion;
  }
   ...
   */
  public String get_module_other_1() {
    return moduleOther1;
  }
   ...
   */
  public String get_module_other_2() {
    return moduleOther2;
  }
  ...
  public String getWindowTitleString(String key, String name, boolean moduleVersion) {
    final String version = get_game_version();
    ...
    if (StringUtils.isEmpty(gameFile) || GameFileMode.NEW_GAME.equals(gameFileMode)) {
      return Resources::get_string(key + "_title", nameString, "", Info.get_version());  //NON-NLS-1$
    }
    else {
      return Resources::get_string(key + "_title_" + gameFileMode, nameString, gameFile, Info.get_version()); //NON-NLS-1$
    }
  }
  ...
  public String getTitleString() {
    return getWindowTitleString("GameModule.frame", get_localized_game_name()); //NON-NLS-1$
  }
  ...
  public void updateTitleBar() {
    ...
    for (final Map m : get_components_of(Map.class)) {
      m.updateTitleBar();
    }
  }
  ...
  public boolean shutDown() {
    ...
    if (!cancelled) {
      if (getDataArchive() instanceof ArchiveWriter
        ...
        switch (JOptionPane.showConfirmDialog(frame,
          Resources::get_string("GameModule.save_module"),  //$NON-NLS-1$
      ...
      for (final ModuleExtension ext : get_components_of(ModuleExtension.class)) {
    ...
    if (!cancelled) {
      ...
      try {
        ...
        ReadErrorDialog.error(e, archive.get_name());
      }
...
*/
  ...
  public static void init(GameModule module) throws IOException {
    if (theModule != null) {
      throw new UnsupportedOperationException(
        Resources::get_string("GameModule.open_error",
          theModule.getDataArchive().get_name()));
    }
     ...
     */
    for (final Plugin plugin : theModule.get_components_of(Plugin.class)) {
      plugin.init();
    }
  }
  ...
  private void checkGpIds() {
    ...
    for (final PrototypesContainer pc : get_components_of(PrototypesContainer.class)) {
  ...
  public ImageTileSource getImageTileSource() {
    if (tcache == null) {
      // FIXME: There's no guarantee that getGameName() and get_game_version()
      // are properly set at this point.

      final String hstr =
        DigestUtils.sha1Hex(getGameName() + "_" + get_game_version()); //NON-NLS
  ...
  private void save(boolean saveAs) {
    vassalVersionCreated = Info.get_version();
    ...
    try {
      ...
      WriteErrorDialog.error(e, writer.get_name());
    }
    ...
    try {
      ...
      if (actuallyDidStuff) {
        ...
        warn(Resources::get_string("Editor.GameModule.saved", writer.getArchive().getFile().get_name()));
      }
    }
    catch (IOException e) {
  ...
  public boolean isRealName() {
    ...
    return name != null && !name.isEmpty() && !name.equals(Resources::get_string("Prefs.newbie"));
  }
  ...
  private void buildCrc() {
    ...
    if (getDataArchive().getArchive() != null) {
      files.add(new File(getDataArchive().get_name()));
    }

    for (final ModuleExtension ext : get_components_of(ModuleExtension.class)) {
      if (ext.getDataArchive().getArchive() != null) {
        files.add(new File(ext.getDataArchive().get_name()));
      }
    }
  ...
  public PlayerRoster getPlayerRoster() {
    return get_components_of(PlayerRoster.class).stream()
                                              .findFirst()
                                              .orElse(null);
  }

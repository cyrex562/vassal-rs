...
class Reflect 
{
		...
		try {
			...
			throw new ReflectError( "Cannot access method " 
				+ StringUtil.methodString(
					method.get_name(), method.getParameterTypes() )
        ...
        throws UtilEvalError, ReflectError
    {
		...
		if ( staticOnly && !Modifier.isStatic( field.getModifiers() ) )
			throw new UtilEvalError(
				...
				+" from static context: "+clas.get_name() );
        ...
        throws ReflectError, UtilEvalError
    {
		...
		if ( method == null )
			throw new ReflectError(
				...
				" not found in class'" + clas.get_name() + "'");
	...
	*/
		...
		int numArgs, boolean publicOnly, Vector candidates  )
	{
		...
		{
			Method m = methods[i];
			if (  m.get_name().equals( methodName )
	...
	*/
		...
		Class clas, String propName )
	{
		...
		for(int i=0; i<methods.length; i++)
			if ( methods[i].get_name().equals( setterName ) )
				return true;
		return false;
	}
    ...
    /**
		...
		would be returned by Class get_name() in that case.
	*/
    public static String normalizeClassName(Class type)
    {
        ...
        try {
            className.append( getArrayBaseType(type).get_name() +" ");
	...
	*/
    public static int getArrayDimensions(Class arrayClass)
    {
        ...
        return arrayClass.get_name().lastIndexOf('[') + 1;  // why so cute?
    }
		...
		if ( types.length == 0 )
			...
			return new ReflectError(
				"Can't find constructor: "
					+ StringUtil.methodString( clas.get_name(), types )
					+" in class: "+ clas.get_name() );
	}

...
public class ModuleExtension extends AbstractBuildable implements GameComponent, PluginsLoader.PluginElement, GpIdSupport {
  ...
  public Command getRestoreCommand() {
    return new RegCmd(get_name(), version);
  }
  ...
  public void removeFrom(Buildable parent) {
    throw new IllegalBuildException(
      Resources::get_string("ModuleExtension.cannot_remove")); //$NON-NLS-1$
  }
  ...
  public boolean confirmExit() {
    ...
    if (archive instanceof ArchiveWriter && !buildString().equals(lastSave)) {
      switch (JOptionPane.showConfirmDialog(
        ...
        Resources::get_string("ModuleExtension.save_extension"), //$NON-NLS-1$
  ...
  public String getAttributeValueString(String key) {
    ...
    if (BASE_MODULE_NAME.equals(key)) {
      ...
      s = GameModule.getGameModule().get_game_version();
    }
    ...
    if (BASE_MODULE_NAME.equals(key)) {
      if (!universal && !GameModule.getGameModule().getGameName().equals(value)) {
        throw new ExtensionsLoader.LoadExtensionException(
          Resources::get_string("ModuleExtension.extension_built", get_name(), value)); //$NON-NLS-1$
      }
    }
    else if (BASE_MODULE_VERSION.equals(key)) {
      final String version = (String) value;
      if (!universal && VersionUtils.compareVersions(GameModule.getGameModule().get_game_version(), version) < 0) {
        GameModule.getGameModule().warn(
            Resources::get_string("ModuleExtension.wrong_module_version", //NON-NLS
                get_name(), version, GameModule.getGameModule().get_game_version(),
  ...
  protected void write(boolean saveAs) throws IOException {
    ...
    if (archive instanceof ArchiveWriter) {
      ...
      GameModule.getGameModule().warn(Resources::get_string("Editor.ExtensionEditor.saved", w.getArchive().getFile().get_name()));
  ...
  public Action getEditAction(final JDialog d) {
    if (editAction == null) {
      ...
      idBox.add(idDisplay);
      final JButton change = new JButton(Resources::get_string("Editor.ExtensionEditor.change_button"));
      change.addActionListener(e -> {
        final String s = (String)JOptionPane.showInputDialog(
            GameModule.getGameModule().getPlayerWindow(),
            Resources::get_string("Editor.ExtensionEditor.change_warning"),
            "", //NON-NLS
      ...
      final JButton ok = new JButton(Resources::get_string("General.save"));
      ...
      if (iconURL != null) {
        ...
        editAction.putValue(Action.NAME, Resources::get_string("General.edit"));
      }
      editAction.putValue(Action.SHORT_DESCRIPTION, Resources::get_string("Editor.ExtensionEditor.extension_properties"));
    }
    return editAction;
  }
  ...
  public static class RegCmd extends Command {
    ...
    protected void executeCommand() {
      ...
      for (final ModuleExtension ext :
           GameModule.getGameModule().get_components_of(ModuleExtension.class)) {
        if (ext.get_name().equals(name)) {
          containsExtension = true;
          if (VersionUtils.compareVersions(ext.get_version(), version) > 0) {
            GameModule.getGameModule().warn(get_versionErrorMsg(ext.get_version()));
          }
          break;
        }
      }
    ...
    protected String get_versionErrorMsg(String v) {
      return Resources::get_string("ModuleExtension.wrong_extension_version", //$NON-NLS-1$
          version, name, v);
    }

    protected String getNotLoadedMsg() {
      return Resources::get_string("ModuleExtension.load_extension", name, new ExtensionsManager(GameModule.getGameModule()).getExtensionsDirectory(false)); //$NON-NLS-1$
    }

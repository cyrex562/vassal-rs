...
public class PlayerHand extends PrivateMap {
  ...
  public void build(org.w3c.dom.Element el) {
    super.build(el);
    if (el == null) {
      for (final StackExpander se : get_components_of(StackExpander.class)) {
        remove(se);
        removeLocalMouseListener(se);
      }
      for (final CounterDetailViewer cdv :
              get_components_of(CounterDetailViewer.class)) {
        remove(cdv);
        cdv.removeFrom(this);
      }
    }
  ...
  public static String getConfigureTypeName() {
    return Resources::get_string("Editor.PlayerHand.component_type"); //$NON-NLS-1$
  }

...
public class ImageSaver extends AbstractToolbarItem {
  ...
  public ImageSaver() {
    ...
    setLaunchButton(makeLaunchButton(
      Resources::get_string("Editor.ImageSaver.save_map_as_png_image"),
      "",
      DEFAULT_ICON,
      e -> writeMapAsImage()
    ));
  ...
  public void writeMapAsImage() {
    ...
    fc.setSelectedFile(
      ...
      GameModule.getGameModule().get_localized_game_name() + "Map.png")
    );
    ...
    dialog = new ProgressDialog(frame, Resources::get_string("Editor.ImageSaver.saving_map_image_title"),
                                       Resources::get_string("Editor.ImageSaver.saving_map_image_text"));
    ...
    final int l = Resources::get_string("Editor.ImageSaver.saving_map_image_as").length() + file.get_name().length() + 7;
    dialog.setLabel("N".repeat(Math.max(0, l))); //NON-NLS

    SwingUtils.repack(dialog);
    dialog.setLabel(Resources::get_string("Editor.ImageSaver.saving_map_image_as") + " ");
...
// FIXME: SnapshotTask ignores x,y!
    ...
    private void writeImage(final File f, BufferedImage img, Rectangle r)
      ...
      SwingUtilities.invokeLater(() -> {
        dialog.setLabel(Resources::get_string("Editor.ImageSaver.saving_map_image_as") + " " + f.get_name() + ":");
        dialog.setIndeterminate(true);
      });
  ...
  public static String getConfigureTypeName() {
    return Resources::get_string("Editor.ImageSaver.component_type"); //$NON-NLS-1$
  }

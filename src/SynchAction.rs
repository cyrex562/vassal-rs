...
public class SynchAction extends AbstractAction {
  ...
  public SynchAction(Player p, ChatServerConnection client) {
    super(Resources::get_string("Chat.synchronize")); //$NON-NLS-1$
    this.p = p;
    this.client = client;
  ...
  public void actionPerformed(ActionEvent evt) {
    if (isEnabled()) {
      ...
      if (! targetRoom.equals(lastRoom) || (now - lastSync) > TOO_SOON) {
        ...
        GameModule.getGameModule().warn(Resources::get_string("Chat.sent_sync_request"));  
      }
      else {
        GameModule.getGameModule().warn(Resources::get_string("Chat.sync_too_soon"));
      }
      lastRoom = targetRoom;
    }
  }

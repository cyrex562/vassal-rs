...
public final class ImportAction extends EditModuleAction {
  ...
  public ImportAction(Component comp) {
    super(comp);
    putValue(NAME, Resources::get_string("Editor.import_module"));
  }
  ...
  public static Class<?> getImporterClass(File f) throws IOException {
    ...
    final String s = '.' + Importer.getExtension(f.get_name());
...
//  }
  ...
  public File getCaseInsensitiveFile(File f, File base,
    ...
    final String name = Importer.getFileName(f.get_name());
    ...
    if (queryIfNotFound) {
      JOptionPane.showMessageDialog(comp, Resources::get_string("Editor.ImportAction.unable", f.getPath()),
          Resources::get_string("Editor.ImportAction.warning"), JOptionPane.WARNING_MESSAGE);
      ...
      fc.setSelectedFile(new File(f.get_name()));

...
public class ChatServerControls extends AbstractBuildable {
  ...
  public ChatServerControls() {
    ...
    roomPanel.setBorder(BorderFactory.createTitledBorder(
      ...
      Resources::get_string("Chat.active_games"))
    );

    newRoom = new JTextField(12);
    newRoomLabel = new JLabel(Resources::get_string("Chat.new_game"));
    newRoomLabel.setLabelFor(newRoom);
    roomPanel.add(newRoomLabel, "");
    roomPanel.add(newRoom, "growx, pushx"); //NON-NLS

    newRoomButton = new JButton(Resources::get_string("Chat.create"));
    ...
    scroll.setBorder(BorderFactory.createTitledBorder(BorderFactory.createRaisedBevelBorder(), Resources::get_string("Chat.current_game")));  //$NON-NLS-1$
  ...
  private void updateConfigServerToolTipText() {
    if (client.isConnected()) {
      ...
      configServerButton.setToolTipText("<html><center>" + configServerText + "<br>" + Resources::get_string("Chat.change_server"));  //NON-NLS
    }
  }
  ...
  public void addTo(Buildable b) {
    ...
    launch = new JButton(Resources::get_string("Chat.server"));  //$NON-NLS-1$
    ...
    final IconConfigurer iconConfig = new IconConfigurer("serverControlsIcon", Resources::get_string("Chat.server_controls_button_icon"), "/images/connect.gif"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    ...
    final NamedHotKeyConfigurer keyConfig = new NamedHotKeyConfigurer("serverControlsHotKey", Resources::get_string("Chat.server_controls_hotkey"), l.get_namedKeyStroke());   //$NON-NLS-1$ //$NON-NLS-2$
    ...
    keyConfig.addPropertyChangeListener(evt -> {
      ...
      launch.setToolTipText(Resources::get_string("Chat.server_controls_tooltip", NamedHotKeyConfigurer.getString(l.getKeyStroke())));  //$NON-NLS-1$
    });
  ...
  public void toggleVisible() {
    if (controlPanel.getTopLevelAncestor() == null) {
      if (GlobalOptions.getInstance().isUseSingleWindow()) {
        ...
        final JFrame frame = new JFrame(Resources::get_string("Chat.server"));  //$NON-NLS-1$

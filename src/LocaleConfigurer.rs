...
public class LocaleConfigurer extends Configurer {
  ...
  public java.awt.Component getControls() {
    if (panel == null) {
      panel = new ConfigurerPanel(get_name(), "[]rel[][]rel[]", "[]rel[]rel[][]rel[]"); // NON-NLS
      ...
      panel.add(new JLabel(Resources::get_string("Editor.LocaleConfigurer.language")));
      panel.add(langBox);

      countryBox = new JComboBox<>(getCountryList());
      countryBox.setSelectedItem(ANY_COUNTRY);
      countryBox.addActionListener(e -> updateValue());
      panel.add(new JLabel((Resources::get_string("Editor.LocaleConfigurer.country"))));

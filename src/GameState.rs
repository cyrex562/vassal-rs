...
public class GameState implements CommandEncoder {
  ...
  public void addTo(@SuppressWarnings("unused") GameModule mod) {
    loadGame = new AbstractAction(Resources::get_string("GameState.load_game_new")) {
    ...
    // some languages
    loadGame.putValue(Action.MNEMONIC_KEY, (int)Resources::get_string("GameState.load_game.shortcut").charAt(0));

    // TODO: remove Load Continuation from File menu as of 3.6
    loadGameOld = new AbstractAction(Resources::get_string("GameState.load_continuation")) {
      private static final long serialVersionUID = 1L;
      ...
      public void actionPerformed(ActionEvent e) {
        ProblemDialog.show(
          ...
          null,
          Resources::get_string("GameState.old_continuation_title"),
          Resources::get_string("GameState.old_continuation_heading"),
          Resources::get_string("GameState.old_continuation_warning")
        );
      }
    };
    loadGameOld.setEnabled(false);

    loadContinuation = new AbstractAction(Resources::get_string("GameState.load_game_old")) {
      private static final long serialVersionUID = 1L;
    ...
    loadAndFastForward = new AbstractAction(Resources::get_string("GameState.load_and_fast_forward")) {
      private static final long serialVersionUID = 1L;
    ...
    loadAndAppend = new AbstractAction(Resources::get_string("GameState.load_and_append")) {
      private static final long serialVersionUID = 1L;
    ...
    saveGame = new AbstractAction(Resources::get_string("GameState.save_game")) {
      private static final long serialVersionUID = 1L;
    ...
    // FIXME: setting mnemonic from first letter could cause collisions in
    // some languages
    saveGame.putValue(Action.MNEMONIC_KEY, (int)Resources::get_string("GameState.save_game.shortcut").charAt(0));

    saveGameAs = new AbstractAction(Resources::get_string("GameState.save_game_as")) {
      private static final long serialVersionUID = 1L;
    ...
    // FIXME: setting mnemonic from first letter could cause collisions in
    // some languages
    saveGameAs.putValue(Action.MNEMONIC_KEY, (int)Resources::get_string("GameState.save_game_as.shortcut").charAt(0));

    newGame = new AbstractAction(Resources::get_string("GameState.new_game")) {
      private static final long serialVersionUID = 1L;
    ...
    // FIXME: setting mnemonic from first letter could cause collisions in
    // some languages
    newGame.putValue(Action.MNEMONIC_KEY, (int)Resources::get_string("GameState.new_game.shortcut").charAt(0));

    closeGame = new AbstractAction(
        Resources::get_string("GameState.close_game")) {
      private static final long serialVersionUID = 1L;
    ...
    // FIXME: setting mnemonic from first letter could cause collisions in
    // some languages
    closeGame.putValue(Action.MNEMONIC_KEY, (int)Resources::get_string("GameState.close_game.shortcut").charAt(0));
  ...
  public int maybeSaveGame() {
    ...
    final int result = JOptionPane.showConfirmDialog(
      GameModule.getGameModule().getPlayerWindow(),
      Resources::get_string("GameState.save_game_query"), //$NON-NLS-1$
      Resources::get_string("GameState.game_modified"),   //$NON-NLS-1$
  ...
  public boolean isSaveMetaDataValid(File file) {
    ...
    final String moduleVersion = GameModule.getGameModule().get_game_version();
    final String vassalVersion = VersionUtils.truncateToMinorVersion(Info.get_version());
    ...
    if (saveData.getModuleData() != null) {
      ...
      if (!saveModuleName.equals(moduleName)) {
        final String m = Resources::get_string("GameState.load_module_mismatch", saveModuleName, moduleName);
        g.warn("!<b>" + m);
        log.info(m);
        final StringBuilder message = new StringBuilder();
        message.append(Resources::get_string("GameState.load_mismatch_header",  file.get_name()))
          .append("\n\n")
          .append(m)
          .append("\n\n")
          .append(Resources::get_string("GameState.load_mismatch_trailer"))
          .append('\n');

        if (JOptionPane.showConfirmDialog(
          null,
          message.toString(),
          Resources::get_string("GameState.load_mismatch"),
      ...
      if (!saveModuleVersion.equals(moduleVersion)) {
        final String message = Resources::get_string("GameState.load_version_mismatch", saveModuleVersion, moduleVersion);
        g.warn("?<b>" + message);
        log.info(message);
      }

      if (!saveVassalVersion.equals(vassalVersion)) {
        final String message = Resources::get_string("GameState.load_vassal_mismatch", saveVassalVersion, vassalVersion);
        g.warn("?<b>" + message);
        log.info(message);
      }
    }
    ...
    if (gameStarted && continuation) {
      if (GlobalOptions.getInstance().isWarnOldContinuation()) {
        final Object[] options = {
          Resources::get_string("GameState.anyway"),
          Resources::get_string("GameState.cancel"),
          Resources::get_string("GameState.dont_prompt_again")
        };
  ...
  public boolean loadGame(File f, boolean continuation, boolean forceForeground) {
    ...
    try {
      ...
      if (!isSaveMetaDataValid(f)) {
        g.warn(Resources::get_string("GameState.cancel_load", f.get_name()));
        return false;
      }
      ...
      if (gameStarted && continuation) {
        ...
        g.setGameFile(f.get_name(), GameModule.GameFileMode.LOADED_GAME);
  ...
  private void loadFastForward(boolean append) {
    ...
    if (bl.isReplaying()) {
      ...
      if (append) {
        if (!bl.isLogging()) {
          g.warn(Resources::get_string("GameState.fast_forward_only"));
        }
        else {
          g.warn(Resources::get_string("GameState.fast_forward_and_append"));
        }
      }
      else {
        if (!bl.isLogging()) {
        ...
        if (!bl.isLogging()) {
          g.warn(Resources::get_string("GameState.fast_forward"));
        }
        else {
          g.warn(Resources::get_string("GameState.fast_forward_new_log"));
        }
      }
    }
    else {
      g.warn(Resources::get_string("GameState.simple_save_append"));
    }
      ...
      if (flavor.isFlavorJavaFileListType()) {
        try {
          ...
          for (final File file : files) {
            if (file.get_name().toLowerCase().endsWith("url")) { // NON-NLS
      ...
      if (md instanceof SaveMetaData) {
        if (Info.hasOldFormat(md.getVassalVersion())) {
          return Dialogs.showConfirmDialog(
            GameModule.getGameModule().getPlayerWindow(),
            Resources::get_string("Warning.save_will_be_updated_title"),
            Resources::get_string("Warning.save_will_be_updated_heading"),
            Resources::get_string(
              "Warning.save_will_be_updated_message",
              f.getPath(),
              VersionUtils.truncateToMinorVersion(Info.get_version())
            ),
    ...
    if (lastSaveFile != null) {
      ...
      try {
        ...
        if (!GameModule.getGameModule().isReplayingOrLogging()) {
          GameModule.getGameModule().setGameFile(lastSaveFile.get_name(), GameModule.GameFileMode.SAVED_GAME);
        }
      }
      catch (IOException e) {
  ...
  public void saveGameAs() {
    ...
    if (saveFile == null) {
      g.warn(Resources::get_string("GameState.save_canceled"));  //$NON-NLS-1$
    }
    else {
      ...
      try {
        ...
        if (!GameModule.getGameModule().isReplayingOrLogging()) {
          g.setGameFile(saveFile.get_name(), GameModule.GameFileMode.SAVED_GAME);
        }
      }
      catch (IOException e) {
  ...
  private File getSaveFile() {
    ...
    if (!file.get_name().endsWith(".vsav")) { //NON-NLS
      file = new File(file.getParent(), file.get_name() + ".vsav"); //NON-NLS
    }
  ...
  public void loadContinuation(File f) throws IOException {
    final GameModule g = GameModule.getGameModule();
    g.warn(Resources::get_string("GameState.loading", f.get_name()));  //$NON-NLS-1$
    ...
    g.setGameFile(f.get_name(), ((BasicLogger)g.getLogger()).isReplaying() ? GameModule.GameFileMode.REPLAYING_GAME : GameModule.GameFileMode.LOADED_GAME);
    String msg = Resources::get_string("GameState.loaded", f.get_name());  //$NON-NLS-1$
  ...
  private Command checkVersionCommand() {
    ...
    final Command c = new ConditionalCommand(new ConditionalCommand.Condition[]{cond}, new AlertCommand(Resources::get_string("GameState.version_mismatch", runningVersion)));  //$NON-NLS-1$
    ...
    c.append(
      new ConditionalCommand(
        ...
        new AlertCommand(Resources::get_string("GameState.version_mismatch2", moduleName, moduleVersion))));  //$NON-NLS-1$
    return c;
  }
  ...
  public void saveGame(File f) throws IOException {
    final SaveMetaData metaData;
    GameModule.getGameModule().warn(Resources::get_string("GameState.saving_game") + ": " + f.get_name());  //$NON-NLS-1$
    // FIXME: It is extremely inefficient to produce the save string. It would
    // be faster to write directly to the output stream instead.
    metaData = new SaveMetaData(); // this also potentially prompts for save file comments, so do *before* possibly long save file write
    ...
    if (save == null) {
      GameModule.getGameModule().warn("~" + Resources::get_string("GameState.save_disabled"));
    }
    ...
    if (!StringUtils.isEmpty(saveComments)) {
      msg = "!" + Resources::get_string("GameState.game_saved") + ": <b>" + saveComments + "</b>"; //$NON-NLS-1$
    }
    else {
      msg = Resources::get_string("GameState.game_saved"); //$NON-NLS-1$
    }
    GameModule.getGameModule().warn(msg);
  ...
  public void loadGameInForeground(final String shortName,
    ...
    final GameModule g = GameModule.getGameModule();
    g.warn(Resources::get_string("GameState.loading", shortName));  //$NON-NLS-1$
    ...
    if (loadCommand != null) {
      try {
        ...
        if (g.getGameState().isGameStarted() || refreshInProgress) {
          if (loadComments != null && loadComments.length() > 0) {
            msg = "!" + Resources::get_string("GameState.loaded", shortName) + ": <b>" + loadComments + "</b>"; //$NON-NLS-1$
          }
          else {
            msg = Resources::get_string("GameState.loaded", shortName); //$NON-NLS-1$
          }
          ...
          msg = Resources::get_string("GameState.cancel_load", shortName);
        }
        g.warn(msg);
      }
    }
  ...
  public void loadGameInBackground(final String shortName,
    ...
    GameModule.getGameModule().warn(
      Resources::get_string("GameState.loading", shortName));  //$NON-NLS-1$
...
// an OOME is neither a VASSAL bug, a module bug, nor due to bad data.
            ...
            msg = Resources::get_string("GameState.error_loading", shortName);
            GameModule.getGameModule().warn(msg);
          }
          ...
          if (g.getGameState().isGameStarted()) {
            if (loadCommand != null) {
              ...
              msg = Resources::get_string("GameState.invalid_savefile", shortName);  //$NON-NLS-1$
              g.setGameFileMode(GameModule.GameFileMode.NEW_GAME);
            }
          }
          else {
  ...
  public Command decodeSavedGame(InputStream in) throws IOException {
    try (ZipInputStream zipInput = new ZipInputStream(in)) {
      for (ZipEntry entry = zipInput.getNextEntry(); entry != null;
        ...
        if (SAVEFILE_ZIP_ENTRY.equals(entry.get_name())) {

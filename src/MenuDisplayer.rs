  ...
  public static JPopupMenu createPopup(GamePiece target, boolean global) {
    ...
    if (c != null) {
      ...
      if (target instanceof Deck) {
        if (c.length == 1) {
          final String menu_item = c[0].get_name();
      ...
      for (final KeyCommand keyCommand : c) {
        ...
        if (keyCommand instanceof KeyCommandSubMenu) {
          ...
          if (strokes.contains(stroke) && !keyCommand.isMenuSeparator()) {
            ...
            if (action != null) {
              ...
              if (commandName == null ||
                      commandName.length() < keyCommand.get_name().length()) {
        ...
        if (keyCommand.get_name() != null &&
            keyCommand.get_name().length() > 0 &&
            item != null) {
          final ArrayList<JMenuItem> l = commandNames.computeIfAbsent(keyCommand.get_name(), k -> new ArrayList<>());
          l.add(item);
        }
      }

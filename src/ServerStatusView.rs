...
public class ServerStatusView extends JTabbedPane implements ChangeListener, TreeSelectionListener {
  ...
  private void initComponents() {
    ...
    final JButton b = new JButton(Resources::get_string("Chat.refresh")); //$NON-NLS-1$
    ...
    addTab(Resources::get_string("Chat.current"), current); //$NON-NLS-1$
    addChangeListener(this);
    setBorder(new TitledBorder(Resources::get_string("Chat.server_connections"))); //$NON-NLS-1$
  ...
  private JTree createTree() {
    final DefaultMutableTreeNode root = new DefaultMutableTreeNode(Resources::get_string(Resources.VASSAL));
  ...
  private void refresh(DefaultTreeModel m,
    ...
    if (modules.length == 0) {
      final DefaultMutableTreeNode n = new DefaultMutableTreeNode(
        Resources::get_string("Chat.no_connections")); //$NON-NLS-1$
    ...
    root.setUserObject(
      Resources::get_string(Resources.VASSAL) + " (" + totalPlayers + ")");
  }
  ...
  public static class Render extends DefaultTreeCellRenderer {
    ...
    public Component getTreeCellRendererComponent(JTree tree, Object value, boolean sel, boolean expanded, boolean leaf, int row, boolean hasFocus) {
      ...
      putClientProperty(Resources::get_string("ServerStatusView.4"), Boolean.TRUE); //$NON-NLS-1$
      return this;
    }
  }

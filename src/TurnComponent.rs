...
public class TurnComponent extends AbstractConfigurable implements ComponentDescription {
  ...
  protected TurnLevel getTurnLevel(int i) {
    if (levels.isEmpty()) {
      ...
      if (i >= levels.size()) {
        ErrorDialog.dataWarning(new BadDataReport(Resources::get_string("TurnTracker.level_error", getConfigureName(), getConfigureName(), i, levels.size()), getConfigureName())); //NON-NLS

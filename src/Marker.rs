...
public class Marker extends Decorator implements EditablePiece {
  ...
  public String getDescription() {
    String result = Resources::get_string("Editor.Marker.trait_description");
    ...
    if (keys != null && keys.length > 0 && keys[0].length() > 0) {
      ...
      if (keys.length > 1) {
        result += " " + Resources::get_string("Editor.Marker.more_keys", keys.length - 1);
      }
    }
  ...
  public String getBaseDescription() {
    return Resources::get_string("Editor.Marker.trait_description");
  }

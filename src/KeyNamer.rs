...
public class KeyNamer implements Buildable {
  ...
  public static String getKeyString(KeyStroke k) {
    ...
    case KeyEvent.VK_ADD:
      sb.append(Resources::get_string("Keys.numplus")); //$NON-NLS-1$ //$NON-NLS-2$
      break;
    case KeyEvent.VK_SUBTRACT:
      sb.append(Resources::get_string("Keys.numminus")); //$NON-NLS-1$ //$NON-NLS-2$
      break;
    // More compact name, also commonly printed keys
    case KeyEvent.VK_PAGE_UP:
      sb.append(Resources::get_string("Keys.pgup")); //$NON-NLS-1$ //$NON-NLS-2$
      break;
    case KeyEvent.VK_PAGE_DOWN:
      sb.append(Resources::get_string("Keys.pgdn")); //$NON-NLS-1$ //$NON-NLS-2$
      break;
    // Non-Americans were (rightly) commenting about this
    case KeyEvent.VK_OPEN_BRACKET:
      sb.append(Resources::get_string("Keys.bropen")); //$NON-NLS-1$ //$NON-NLS-2$
      break;
    case KeyEvent.VK_CLOSE_BRACKET:
      sb.append(Resources::get_string("Keys.brclose")); //$NON-NLS-1$ //$NON-NLS-2$
      break;
    default:
      break;
    }
    ...
    if ((mods & KeyEvent.SHIFT_DOWN_MASK) != 0) {
      ...
      sb.insert(0, Resources::get_string("Keys.shift")); //$NON-NLS-1$
    }
    if ((mods & KeyEvent.CTRL_DOWN_MASK) != 0) {
      sb.insert(0, "+"); //$NON-NLS-1$
      sb.insert(0, Resources::get_string("Keys.ctrl")); //$NON-NLS-1$
    }
    if ((mods & KeyEvent.META_DOWN_MASK) != 0) {  // This is "Command" key on Mac
      sb.insert(0, "+"); //$NON-NLS-1$
      sb.insert(0, Resources::get_string("Keys.meta")); //$NON-NLS-1$
    }
    if ((mods & KeyEvent.ALT_DOWN_MASK) != 0) {
      sb.insert(0, "+"); //$NON-NLS-1$
      sb.insert(0, Resources::get_string("Keys.alt")); //$NON-NLS-1$
    }

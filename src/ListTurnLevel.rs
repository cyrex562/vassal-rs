...
public class ListTurnLevel extends TurnLevel implements ActionListener {
  ...
  protected void buildConfigMenu(JMenu configMenu) {
    ...
    if (configFirst || configList) {
      final JMenuItem item = new JMenuItem(Resources::get_string("TurnTracker.configure2", getConfigureName())); //$NON-NLS-1$
  ...
  public String[] getAttributeDescriptions() {
    return ArrayUtils.addAll(
      ...
      Resources::get_string("Editor.ListTurnLevel.list_of_items"),
      Resources::get_string("Editor.ListTurnLevel.allow_hide"),
      Resources::get_string("Editor.ListTurnLevel.allow_reorder"),
      Resources::get_string("Editor.ListTurnLevel.prompt_reorder")
    );
  }
  ...
  public static String getConfigureTypeName() {
    return Resources::get_string("Editor.ListTurnLevel.component_type");
  }
  ...
  protected class ConfigDialog extends JDialog {
    ...
    public ConfigDialog() {
      super(GameModule.getGameModule().getPlayerWindow(), Resources::get_string("TurnTracker.configure2", getConfigureName())); //$NON-NLS-1$
      setLayout(new MigLayout("wrap 2", "[center]rel[left]"));

      if (configFirst) {
        if (prompt == null) {
          prompt = Resources::get_string("Editor.ListTurnLevel.first_each", getConfigureName(), parent.getConfigureName());
        }
      ...
      if (configList) {
        ...
        add(new JLabel(Resources::get_string("TurnTracker.turn_off")), "span 2"); //$NON-NLS-1$
        for (int i = 0; i < list.length; i++) {
          ...
          b.addPropertyChangeListener(e -> {
            ...
            final String option = b1.get_name();

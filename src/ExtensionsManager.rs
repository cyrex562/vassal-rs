...
public class ExtensionsManager {
  ...
  public ExtensionsManager(GameModule module) {
    this.moduleFile = new File(module.getDataArchive().get_name());
  }
  ...
  public File setActive(File extension, boolean active) {
    ...
    if (active) {
      ...
      newExt = new File(extensionsDirectory, extension.get_name());
    }
    else {
      ...
      newExt = new File(inactiveExtensionsDirectory, extension.get_name());
    }
  ...
  public boolean isExtensionActive(File extension) {
    for (final File f : getActiveExtensions()) {
      if (f.get_name().equals(extension.get_name())) {
        return true;
      }
    }
    return false;

  ...
  public void initializeControls(final ChatServerControls controls) {
    ...
    roomCreator = e -> {
      if (!client.isConnected()) {
        GameModule.getGameModule().warn(Resources::get_string("Chat.connect_first"));
        return;
      }
      if ("".equals(controls.getNewRoom().getText())) {
        GameModule.getGameModule().warn(Resources::get_string("Chat.name_first"));
        return;
      }
      createRoom(controls.getNewRoom().getText());

      GameModule.getGameModule().warn(Resources::get_string("Chat.creating_room", controls.getNewRoom().getText()));
      GameModule.getGameModule().warn(Resources::get_string("Chat.explain_created_room"));

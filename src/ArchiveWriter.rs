...
public class ArchiveWriter extends DataArchive {
  ...
  public void addImage(String path, String name) {
    ...
    if (name.toLowerCase().endsWith(".svg")) { //$NON-NLS-1$//
      ...
      for (final String s : exrefs) {
        ...
        addFile(imageDir + f.get_name(), buf);
      }
    }
  ...
  public void removeFile(String name) {
    try {
      ...
      WriteErrorDialog.error(e, archive.get_name());
    }
  }
  ...
  public boolean saveAsButVerify(boolean notifyModuleManager) throws IOException {
    ...
    if (!StringUtils.isEmpty(defaultExtension) && (filename.lastIndexOf('.') < 0)) {
      ...
      if (new File(filename).exists() && JOptionPane.NO_OPTION == JOptionPane.showConfirmDialog(GameModule.getGameModule().getPlayerWindow(), Resources::get_string("Editor.ArchiveWriter.overwrite", filename), Resources::get_string("Editor.ArchiveWriter.file_exists"), JOptionPane.YES_NO_OPTION)) {
        return false;
      }
    }

    if (!filename.equals(archive.get_name())) {

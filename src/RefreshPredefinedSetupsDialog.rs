...
public class RefreshPredefinedSetupsDialog extends JDialog {
  ...
  public RefreshPredefinedSetupsDialog(Frame owner) throws HeadlessException {
    super(owner, false);
    setTitle(Resources::get_string("Editor.RefreshPredefinedSetupsDialog.title"));
    initComponents();
  }
  ...
  private void initComponents() {
    ...
    final FlowLabel header = new FlowLabel(Resources::get_string("GameRefresher.predefined_header"));
    panel.add(header);

    final JPanel buttonsBox = new JPanel(new MigLayout("ins 0", "push[]rel[]rel[]push")); // NON-NLS
    refreshButton = new JButton(Resources::get_string("General.run"));
    ...
    final JButton closeButton = new JButton(Resources::get_string("General.cancel"));
  ...
  private void refreshPredefinedSetups() {
    ...
    setOptions();
    if (isTestMode()) {
      log(Resources::get_string("GameRefresher.refresh_counters_test_mode"));
    }
    ...
    final List<ModuleExtension>  moduleExtensionList = mod.get_components_of(ModuleExtension.class);
    ...
    log(modulePds.size() + " " + Resources::get_string("GameRefresher.predefined_setups_found"));

...
public class NewColorConfigurer extends Configurer {
  ...
  public java.awt.Component getControls() {
    if (p == null) {
      ...
      box.add(new JLabel(Resources::get_string("Editor.NewColorConfigurer.use_named_colors")));
      ...
      colorBox.add(new JLabel(get_name()));
      cp = new Panel();
      ...
      colorBox.add(cp);
      final JButton b = new JButton(Resources::get_string("Editor.select"));
      colorBox.add(b);
      p.add(colorBox);

      b.addActionListener(e -> {
        setValue(JColorChooser.showDialog(null, get_name(), colorValue()));
        csc.setValue(new ColorSwatch("", (Color) getValue())); //$NON-NLS-1$
      });

      swatchBox = Box.createHorizontalBox();
      csc = new ColorSwatchConfigurer(null, Resources::get_string("Editor.NewColorConfigurer.select_color"), "WHITE"); //$NON-NLS-2$

...
public class IconFamily extends AbstractConfigurable {
  ...
  public static String[] getIconSizeNames() {
    synchronized (SIZE_NAMES) {
      if (SIZE_NAMES[0] == null) {
        SIZE_NAMES[XSMALL] = Resources::get_string("Icon.extra_small"); //$NON-NLS-1$
        SIZE_NAMES[SMALL] = Resources::get_string("Icon.small"); //$NON-NLS-1$
        SIZE_NAMES[MEDIUM] = Resources::get_string("Icon.medium"); //$NON-NLS-1$
        SIZE_NAMES[LARGE] = Resources::get_string("Icon.large"); //$NON-NLS-1$
      }
    }
    return SIZE_NAMES;
  }
  ...
  public boolean isLegacy() {
    return get_name().contains("."); //$NON-NLS-1$
  }
  ...
  public static String getConfigureTypeName() {
    return Resources::get_string("Editor.IconFamily.component_type"); //$NON-NLS-1$
  }
  ...
  static class IconFamilyConfig extends Configurer {
    ...
    public IconFamilyConfig(IconFamily f) {
      ...
      mig.add(new JLabel(Resources::get_string("Editor.IconFamily.family_name"))); //$NON-NLS-1$
      mig.add(title.getControls(), "wrap"); //$NON-NLS-1$

      errorLabel = new JLabel(Resources::get_string("Editor.IconFamily.name_taken")); //$NON-NLS-1$
      ...
      mig.add(new JLabel(Resources::get_string("Editor.IconFamily.scalable_icon_label"))); //$NON-NLS-1$
      ...
      for (int size = 0; size < IconFamily.SIZE_COUNT; size++) {
        ...
        mig.add(new JLabel(Resources::get_string("Editor.IconFamily.icon_label", IconFamily.getIconSizeNames()[size], px))); //$NON-NLS-1$
  ...
  static class IconImageConfigurer extends Configurer {
    ...
    public Component getControls() {
      if (controls == null) {
        ...
        controls.add(new JLabel(get_name()));
        ...
        final JButton select = new JButton(Resources::get_string(Resources.SELECT));
    ...
    protected void checkIconSize() {
      ...
      if (check != null) {
        if (check.getIconHeight() != IconFamily.SIZES[size]) {
          setWarning(Resources::get_string("Editor.IconFamily.size_warning", IconFamily.SIZES[size], check.getIconHeight())); //$NON-NLS-1$
        }
      }
    }
    ...
    protected void selectImage() {
      ...
      fc.setFileFilter(new FamilyImageFilter(family.get_name()));
      fc.setSelectedFile(new File(family.get_name() + ".*")); //$NON-NLS-1$
      if (fc.showOpenDialog(getControls()) != FileChooser.APPROVE_OPTION) {
        ...
        if (f != null && f.exists()) {
          final String name = f.get_name();
          if (name.split("\\.").length != 2) { //$NON-NLS-1$
            showError(Resources::get_string("Editor.IconFamily.illegal_icon_name")); //$NON-NLS-1$
          }
          else if (!name.startsWith(family.get_name())) {
            showError(Resources::get_string("Editor.IconFamily.bad_icon_name", family.get_name())); //$NON-NLS-1$
          }
          else if (!ImageUtils.hasImageSuffix(name)) {
            showError(Resources::get_string("Editor.IconFamily.bad_icon_file")); //$NON-NLS-1$
          }
          else {
            GameModule.getGameModule().getArchiveWriter().addImage(f.getPath(),
                buildPath(f.get_name()));
            setWarning(""); //$NON-NLS-1$
            setValue(name);
          }
        }
    ...
    protected void showError(String message) {
      Dialogs.showMessageDialog(SwingUtilities.getWindowAncestor(controls),
          Resources::get_string("Editor.IconFamily.icon_load_error"),  //$NON-NLS-1$
          Resources::get_string("Editor.IconFamily.cannot_load_icon"), message, //$NON-NLS-1$
  ...
  static class FamilyImageFilter extends ImageFileFilter {
    ...
    public boolean accept(File f) {
      if (super.accept(f)) {
        final String s = f.get_name().split("\\.")[0]; //$NON-NLS-1$

...
public class PlayerInfoWindow extends JDialog {
  ...
  public PlayerInfoWindow(Frame f, SimplePlayer p) {
    ...
    // player name
    final JTextField name_f = new JTextField(p.get_name().length());
    name_f.setText(p.get_name());
    name_f.setEditable(false);

    final JLabel name_l = new JLabel(Resources::get_string("Chat.real_name")); //$NON-NLS-1$
    ...
    if (ip != null && ip.length() > 0) {
      ...
      final JLabel ip_l = new JLabel(Resources::get_string("Chat.ip_address")); //$NON-NLS-1$
    ...
    final JLabel client_l =
      new JLabel(Resources::get_string("Chat.client_version")); //$NON-NLS-1$
    ...
    if (moduleVersion != null && moduleVersion.length() > 0) {
      ...
      final JLabel mver_l =
        new JLabel(Resources::get_string("Chat.module_version")); //$NON-NLS-1$
    ...
    if (csum != null && csum.length() > 0) {
      ...
      final JLabel csum_l =
        new JLabel(Resources::get_string("Chat.module_checksum")); //$NON-NLS-1$
    ...
    if (c2sum != null && ! c2sum.isEmpty()) {
      ...
      final JLabel csum_l = new JLabel(Resources::get_string("Chat.combined_checksum")); //$NON-NLS-1$
      csum_l.setLabelFor(csum_f);
    ...
    final JCheckBox looking_b =
      new JCheckBox(Resources::get_string("Chat.looking_for_a_game")); //$NON-NLS-1$
    ...
    final JCheckBox away_b =
      new JCheckBox(Resources::get_string("Chat.away_from_keyboard")); //$NON-NLS-1$
    ...
    final JLabel pinfo_l =
      new JLabel(Resources::get_string("Chat.personal_info")); //$NON-NLS-1$

...
public class UsernameAndPasswordDialog extends JDialog {
  ...
  public UsernameAndPasswordDialog(Frame parent) {
    super(parent, Resources::get_string("Editor.UsernameAndPasswordDialog.choose_your_weapons"), true);
    setLocationRelativeTo(parent);
    setDefaultCloseOperation(JDialog.DISPOSE_ON_CLOSE);

    setLayout(new MigLayout("ins panel,gapy 4,wrap 2", "[right]rel[fill,grow]")); // NON-NLS

    add(new JLabel(Resources::get_string("WizardSupport.RealName")));
    final StringConfigurer nameConfig = new StringConfigurer("");
    add(nameConfig.getControls());

    add(new JLabel(Resources::get_string("WizardSupport.Password")));
    final StringConfigurer pwd = new PasswordConfigurer(null, "");
    add(pwd.getControls());

    add(new JLabel(Resources::get_string("WizardSupport.ConfirmPassword")));
    final StringConfigurer pwd2 = new PasswordConfigurer(null, "");
    add(pwd2.getControls());

    final JLabel note = new JLabel(Resources::get_string("WizardSupport.Details"));
    add(note, "span 2,center"); // NON-NLS

    final JLabel error = new JLabel(Resources::get_string("WizardSupport.EnterNameAndPassword"));
    ...
    final PropertyChangeListener pl = evt -> {
      if (nameConfig.getValue() == null ||
        ...
        if (pwd.getValue() == null || "".equals(pwd.getValue())) {
          error.setText(Resources::get_string(
        ...
        error.setText(Resources::get_string("Editor.UsernameAndPasswordDialog.turn_your_key_sir"));
        error.setForeground(Color.black);
        ok.setEnabled(false);
      }

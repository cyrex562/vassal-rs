...
public class PieceSlot extends Widget implements MouseListener, KeyListener {
  ...
  public void setPiece(GamePiece p) {
    ...
    if (c != null) {
      ...
      name = Decorator.getInnermost(c).get_name();
    }
    panel.revalidate();
    panel.repaint();
  ...
  public static String getConfigureTypeName() {
    return Resources::get_string("Editor.PieceSlot.component_type");
  }
  ...
  public String getConfigureName() {
    if ((name != null) && !name.isEmpty()) {
      ...
      return Decorator.getOutermost(getExpandedPiece()).get_name();
    }
    else {

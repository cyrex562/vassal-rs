		...
		if ( type == FIELD )
		{
			try {
    			...
    			throw new UtilEvalError(
					"LHS ("+field.get_name()+") not a static field.");
			}     
   			catch( IllegalAccessException e2) {   
				throw new UtilEvalError(
					"LHS ("+field.get_name()+") can't access field: "+e2);
			}     
			catch( IllegalArgumentException e3) 
			{
				String type = val instanceof Primitive ?
					((Primitive)val).getType().get_name()
					: val.getClass().get_name();
				throw new UtilEvalError(
					"Argument type mismatch. " + (val == null ? "null" : type )
					+ " not assignable to field "+field.get_name());
			}
		}
		else 
		if ( type == PROPERTY )

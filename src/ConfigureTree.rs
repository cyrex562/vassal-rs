...
public class ConfigureTree extends JTree implements PropertyChangeListener, MouseListener, MouseMotionListener, TreeSelectionListener, TreeExpansionListener {
  ...
  public ConfigureTree(Configurable root, HelpWindow helpWindow, EditorWindow editorWindow) {
    ...
    searchCmd = Resources::get_string("Editor.search"); //$NON-NLS-1$
    moveCmd = Resources::get_string("Editor.move"); //$NON-NLS-1$
    deleteCmd = Resources::get_string("Editor.delete"); //$NON-NLS-1$
    pasteCmd = Resources::get_string("Editor.paste"); //$NON-NLS-1$
    copyCmd = Resources::get_string("Editor.copy"); //$NON-NLS-1$
    cutCmd = Resources::get_string("Editor.cut"); //$NON-NLS-1$
    propertiesCmd = Resources::get_string("Editor.properties"); //$NON-NLS-1$
    translateCmd = Resources::get_string("Editor.ModuleEditor.translate"); //$NON-NLS-1$
    helpCmd = Resources::get_string("Editor.ModuleEditor.component_help"); //$NON-NLS-1$
    duplicateCmd = Resources::get_string("Editor.duplicate");
  ...
  protected JPopupMenu buildPopupMenu(final Configurable target) {
    ...
    addSubMenu(popup, Resources::get_string("Editor.ConfigureTree.insert"), inserts);
    ...
    if (getTreeNode(target).getParent() != null) {
      a = new AbstractAction(moveCmd) {
        ...
        public void actionPerformed(ActionEvent e) {
          ...
          box.add(new JLabel(Resources::get_string("Editor.ConfigureTree.move_to_position")));
          ...
          box.add(select);
          final JButton ok = new JButton(Resources::get_string(Resources.OK));
    ...
    if (target instanceof SetupStack) {
      ...
      if (owning != null) {
        if (!ss.getValidOwningBoards().contains(owning)) {
          ConfigureTree.chat(Resources::get_string("Editor.convert_setupstack_or_deck", (target instanceof DrawPile) ? DrawPile.getConfigureTypeName() : SetupStack.getConfigureTypeName(), ss.getConfigureName(), owning));
      ...
      public void actionPerformed(ActionEvent e) {
        if (cutData != null) {
          ...
          if (targetNode.isNodeAncestor(cutData)) {
            chat(Resources::get_string("Editor.cant_cut_ancestor_to_child"));
            return;
          }
  ...
  protected Action buildImportAction(final Configurable target) {
    return new AbstractAction(Resources::get_string("Editor.ConfigureTree.add_imported_class")) {
      private static final long serialVersionUID = 1L;
  ...
  protected Action buildMassPieceLoaderAction(final Configurable target) {
    ...
    if (getTreeNode(target).getParent() != null) {
      Resources::get_string("Editor.ConfigureTree.add_cards");
      final String desc = hasChild(target, CardSlot.class) ? Resources::get_string("Editor.ConfigureTree.add_cards") : Resources::get_string("Editor.ConfigureTree.add_pieces");
  ...
  protected Action buildAddAction(final Configurable target, final Class<? extends Buildable> newConfig, String key, int index, final Configurable duplicate) {
    return new AbstractAction(Resources::get_string(key, getConfigureName(newConfig))) {
      private static final long serialVersionUID = 1L;
  ...
  protected Action buildCloneAction(final Configurable target) {
    ...
    if (targetNode.getParent() != null) {
      return new AbstractAction(Resources::get_string("Editor.ConfigureTree.clone")) {
        private static final long serialVersionUID = 1L;
  ...
  public static String getConfigureName(Class<?> c) {
    ...
    return c.get_name().substring(c.get_name().lastIndexOf(".") + 1);
  }
  ...
  protected Configurable importConfigurable() {
    final String className = JOptionPane.showInputDialog(
      getTopLevelAncestor(),
      Resources::get_string("Editor.ConfigureTree.java_name"));
    ...
    if (targetNode != null) {
      ...
      if (cutCmd.equals(action)) {
        ...
        if (targetNode2 != null) {
          ...
          if (parentNode != null) {
            a = buildAddAction((Configurable) parentNode.getUserObject(), target.getClass(), Resources::get_string("Editor.duplicate"), parentNode.getIndex(targetNode2) + 1, target);
          }
        }
      }
      if (a != null) {
  ...
  private static class SearchAction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      if (d != null) {
        ...
        search = new HintTextField(32, Resources::get_string("Editor.search_string"));
        search.setText(searchParameters.getSearchString());
        configureTree.setSearchField(search);
        search.selectAll();

        final JCheckBox sensitive = new JCheckBox(Resources::get_string("Editor.search_case"), searchParameters.isMatchCase());
        final JCheckBox advanced  = new JCheckBox(Resources::get_string("Editor.search_advanced"), searchParameters.isMatchAdvanced());

        final JCheckBox names = new JCheckBox(Resources::get_string("Editor.search_names"), searchParameters.isMatchNames());
        final JCheckBox types = new JCheckBox(Resources::get_string("Editor.search_types"), searchParameters.isMatchTypes());

        final JCheckBox traits = new JCheckBox(Resources::get_string("Editor.search_traits"), searchParameters.isMatchTraits());
        final JCheckBox expressions = new JCheckBox(Resources::get_string("Editor.search_expressions"), searchParameters.isMatchExpressions());
        final JCheckBox properties = new JCheckBox(Resources::get_string("Editor.search_properties"), searchParameters.isMatchProperties());

        final JCheckBox keys = new JCheckBox(Resources::get_string("Editor.search_keys"), searchParameters.isMatchKeys());
        final JCheckBox menus = new JCheckBox(Resources::get_string("Editor.search_menus"), searchParameters.isMatchMenus());
        final JCheckBox messages = new JCheckBox(Resources::get_string("Editor.search_messages"), searchParameters.isMatchMessages());
        ...
        final JButton find = new JButton(Resources::get_string("Editor.search_next"));
        find.addActionListener(e12 -> {
          ...
          if (!searchParameters.isMatchNames() && !searchParameters.isMatchTypes() && searchParameters.isMatchAdvanced() && (!searchParameters.isMatchTraits() && !searchParameters.isMatchExpressions() && !searchParameters.isMatchProperties() && !searchParameters.isMatchKeys() && !searchParameters.isMatchMenus() && !searchParameters.isMatchMessages())) {
            ...
            ConfigureTree.chat(Resources::get_string("Editor.search_all_off"));
          }
          ...
          if (!searchParameters.getSearchString().isEmpty()) {
            if (anyChanges) {
              ...
              chat(matches + " " + Resources::get_string("Editor.search_count") + noHTML(searchParameters.getSearchString()));
            }
            ...
            if (node != null) {
              ...
              chat(Resources::get_string("Editor.search_none_found") + noHTML(searchParameters.getSearchString()));
            }
          }
        });

        final JButton cancel = new JButton(Resources::get_string(Resources.CANCEL));
        cancel.addActionListener(e1 -> configureTree.getSearchDialog().setVisible(false));

        final JButton help = new JButton(Resources::get_string(Resources.HELP));
    ...
    private boolean checkSearchTarget(SearchTarget st, String searchString) {
      ...
      if (searchParameters.isMatchKeys()) {
        final List<NamedKeyStroke> keys = st.get_namedKeyStrokeList();
        if (keys != null) {
          for (final NamedKeyStroke k : keys) {
            if (k != null) {
              final String s = k.isNamed() ? k.get_name() : KeyNamer.getKeyString(k.getStroke());
              if (!StringUtils.isEmpty(s) && checkString(s, searchString)) {
                return true;
              }
            }
    ...
    private void showConfigurableHitList(DefaultMutableTreeNode node, String searchString) {
      ...
      keyListHits(searchParameters.isMatchKeys(),           st.get_namedKeyStrokeList(),  searchString, matchString, item, "", "KeyCommand",    progress); //NON-NLS
    }
    ...
    private void showHitList(DefaultMutableTreeNode node, String searchString) {
      ...
      for (final GamePiece piece : pieces) {
        if (!protoskip && (piece instanceof EditablePiece) && (piece instanceof Decorator)) { // Skip the fake "Basic Piece" on a Prototype definition
          ...
          keyListHits(searchParameters.isMatchKeys(), d.get_namedKeyStrokeList(), searchString, matchString, "Trait", desc, "KeyCommand", progress); //NON-NLS
        }
        protoskip = false;
      }
    }
    ...
    public TreeTransferHandler() {
      try {
        final String mimeType = DataFlavor.javaJVMLocalObjectMimeType +
          ";class=\"" +
          DefaultMutableTreeNode[].class.get_name() +
          "\"";

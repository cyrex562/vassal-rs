...
public class FileConfigurer extends Configurer {
  ...
  public String getValueString() {
    if (archive == null) {
      ...
      return getFileValue() == null ? "null" : getFileValue().get_name(); // NON-NLS
    }
  }
...
// FIXME: this creates a problem when the referenced file is in the JAR
  ...
  protected void addToArchive(File f) {
    archive.addFile(f.getPath(), f.get_name());
  }
  ...
  public Component getControls() {
    if (p == null) {
      p = new ConfigurerPanel(get_name(), "[]rel[grow,fill]", "[]rel[]rel[grow,fill]"); // NON-NLS

  ...
  protected void executeCommand() {
    if (client instanceof NodeClient) {
      final int i = Dialogs.showConfirmDialog(
        ...
        Resources::get_string("Chat.invite_heading"), //$NON-NLS-1$
        Resources::get_string("Chat.invite_heading"), //$NON-NLS-1$
        Resources::get_string("Chat.invitation", player, room),  //$NON-NLS-1$
        JOptionPane.QUESTION_MESSAGE,
        null,
        JOptionPane.YES_NO_OPTION,
        "Invite" + playerId,  //$NON-NLS-1$
        Resources::get_string("Chat.ignore_invitation") //$NON-NLS-1$
      );

...
public class NamedKeyStrokeArrayConfigurer extends Configurer implements ConfigurableList {
  ...
  public NamedKeyStroke[] get_nameKeyStrokeArrayValue() {
  ...
  private void rebuildControls() {
    ...
    final NamedKeyStroke[] keys = get_nameKeyStrokeArrayValue();
  ...
  private void setKeyValue(int pos, NamedKeyStroke stroke) {
    final NamedKeyStroke[] oldValue = ArrayUtils.clone(get_nameKeyStrokeArrayValue());
    get_nameKeyStrokeArrayValue()[pos] = stroke;
    if (!frozen) {
      changeSupport.firePropertyChange(key, oldValue, get_nameKeyStrokeArrayValue());
    }
  }
  ...
  public void moveEntryUp() {
    ...
    NamedKeyStroke[] keys = getValue() == null ? new NamedKeyStroke[0] : get_nameKeyStrokeArrayValue();
  ...
  public void addEntry() {
    ...
    final int pos = getSelectedEntryIndex();
    final NamedKeyStroke[] keys = getValue() == null ? new NamedKeyStroke[0] : get_nameKeyStrokeArrayValue();
    ...
    if (entries.isEmpty() || getSelectedEntryIndex() < 0) {
      ...
      newEntry = get_nameKeyStrokeArrayValue().length - 1;

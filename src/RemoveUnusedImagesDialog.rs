...
public class RemoveUnusedImagesDialog extends JDialog {
  ...
  public RemoveUnusedImagesDialog(Frame owner) {
    super(owner, Resources::get_string("Editor.UnusedImages.remove_unused_images"), true);

    final FlowLabel text =
      new FlowLabel(Resources::get_string("Editor.UnusedImages.unused_1"));
    ...
    keepScroll.setBorder(BorderFactory.createTitledBorder(Resources::get_string("Editor.UnusedImages.files_to_keep")));

    final JList<String> dropList = new JList<>(dumpModel);
    final JScrollPane dropScroll = new JScrollPane(dropList);
    dropScroll.setBorder(BorderFactory.createTitledBorder(Resources::get_string("Editor.UnusedImages.files_to_remove")));
    ...
    ok = new JButton(Resources::get_string("Editor.UnusedImages.remove_files"));
  ...
  private void removeImages() {
    for (final String uName : dump) {
      ...
      GameModule.getGameModule().warn("- " + Resources::get_string("Editor.UnusedImages.removing", uName));
